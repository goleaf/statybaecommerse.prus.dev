...
<div class="container mx-auto px-4 py-8" wire:loading.attr="aria-busy" aria-busy="false">
    <a href="#results" class="sr-only focus:not-sr-only focus:underline">{{ __('translations.skip_to_results') }}</a>
    @if (session('status'))
    ...
    @endif
    <x-breadcrumbs :items="[
        ['label' => __('frontend/collections.page_title'), 'url' => route('localized.collections.index', ['locale' => app()->getLocale()])],
    ...
    <div class="flex items-center justify-between mb-6">
        ...
        <div class="flex items-center gap-2">
            <label for="sort" class="sr-only">{{ __('frontend/collections.sort.label') }}</label>
            <select id="sort" wire:model.live="sort" class="rounded-md border-gray-300 text-sm">
                <option value="">{{ __('frontend/collections.sort.newest') }}</option>
                <option value="name_asc">{{ __('frontend/collections.sort.name_asc') }}</option>
                <option value="name_desc">{{ __('frontend/collections.sort.name_desc') }}</option>
            </select>
        </div>
    </div>

    <div wire:loading role="status" aria-live="polite" class="mb-4 text-sm text-gray-600">
        {{ __('translations.loading') }}
    </div>
    ...
    <div class="mb-4">
        <h2 class="text-xl font-semibold mb-2">{{ __('frontend/collections.filters.by_brand') }}</h2>
        <div class="flex flex-wrap items-center gap-2 mb-2">
            ...
            @if (collect($brandIds)->filter()->isNotEmpty())
                <button type="button" wire:click="clearBrandFilters" 
                    ...
                    {{ __('frontend/collections.filters.clear_all') }}
                </button>
            @endif
        </div>
    ...
    @if (isset($options) && $options->isNotEmpty())
        <div class="mb-6">
            <h2 class="text-xl font-semibold mb-2">{{ __('frontend/collections.filters.by') }}</h2>
            <div class="flex flex-wrap items-center gap-2 mb-2">
    ...
    <h2 class="text-xl font-semibold mb-4">{{ __('translations.products') }}</h2>
