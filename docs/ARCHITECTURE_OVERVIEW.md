...
- `app/Support/` — Helper classes and traits shared across services/resources, including caching helpers, locale utilities, and feature toggles. The new `App\Support\Nav` helper centralises Filament navigation metadata by statically parsing resource files and optionally accepting an injected translator when tooling runs outside Laravel's container.
