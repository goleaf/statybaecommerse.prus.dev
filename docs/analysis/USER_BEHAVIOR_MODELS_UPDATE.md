...
- Admin guard seeders now create `AdminUser` records for `superuser@example.com` and `admin@example.com` using the shared `admin123` password, ensuring Filament authentication succeeds even when only the admin guard is enabled.【F:database/seeders/AdminUserSeeder.php†L1-L54】
