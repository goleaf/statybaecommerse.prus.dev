...
- Refined `UserProductInteraction` meta handling to accept legacy payload shapes, propagate rating/count columns, emit fully JSON-encoded payloads, and auto-populate missing `first_interaction`/`last_interaction` values from the resolved `occurred_at` timestamp so downstream analytics consumers receive typed, chronologically complete records.【F:app/Models/UserProductInteraction.php†L31-L420】
