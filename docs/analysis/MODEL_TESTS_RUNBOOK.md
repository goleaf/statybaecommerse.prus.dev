...
- Filament resources that previously lacked smoke coverage are now consolidated in `tests/Feature/Filament/Resources/MissingFilamentResourceCoverageTest.php`; update the data provider when introducing new admin resources so the shared Livewire list assertions stay comprehensive. The suite now also exercises campaign conversions, cart items, legal listings, recommendation config variants, system setting categories, and user wishlists so duplicated list classes stay green across the v4 namespace shuffle.【F:tests/Feature/Filament/Resources/MissingFilamentResourceCoverageTest.php†L7-L594】
