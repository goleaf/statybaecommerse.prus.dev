...
8. Capture any non-obvious insights or new invariants in the relevant markdown audits inside `docs/analysis/` so future tasks have the context baked in.
   ...
   - Product resource unit coverage now confirms the schema fallback injects the testing Livewire host while respecting pre-assigned hosts, keeping Filament v4 form builders operable in isolation tests.【F:tests/Unit/Filament/ProductResourceFormTest.php†L7-L71】
