...
- Referral programme administration now ships dedicated Livewire suites for rewards, reward logs, and aggregated statistics (`ReferralRewardResourceTest`, `ReferralRewardLogResourceTest`, `ReferralStatisticsResourceTest`). Mirror their approach—booting the admin panel, pinning locales to English, and calling `loadTable()` before asserting table state or triggering actions—when extending the referral tooling.【F:tests/Feature/Filament/Resources/ReferralRewardResourceTest.php†L7-L78】【F:tests/Feature/Filament/Resources/ReferralRewardLogResourceTest.php†L7-L83】【F:tests/Feature/Filament/Resources/ReferralStatisticsResourceTest.php†L7-L86】
