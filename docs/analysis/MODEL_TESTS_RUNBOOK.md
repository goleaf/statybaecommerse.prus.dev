...
- Filament resources that previously lacked smoke coverage are now consolidated in `tests/Feature/Filament/Resources/MissingFilamentResourceCoverageTest.php`; update the data provider when introducing new admin resources so the shared Livewire list assertions stay comprehensive.【F:tests/Feature/Filament/Resources/MissingFilamentResourceCoverageTest.php†L7-L205】
  - Additional Filament v4 list pages with namespace variations (news tags/images, recommendation config simples, system setting categories/translations, and user wishlists) now have dedicated smoke coverage in `tests/Feature/Filament/Resources/AdditionalFilamentResourceCoverageTest.php`; extend that suite when adding further dual-namespace list pages so both legacy and refactored resources stay exercised.【F:tests/Feature/Filament/Resources/AdditionalFilamentResourceCoverageTest.php†L1-L134】
