...
2. Execute the model test suite. Running the folder keeps execution time reasonable while covering the bulk of behavioural contracts:
   ...
   - A dedicated factory smoke test (`tests/Unit/Database/AllFactoriesTest.php`) now walks every class in `database/factories` and inspects the raw definition payload to guarantee the blueprints stay valid without bespoke setup. Keep new factories autoloadable and avoid abstract definitions so the reflection-based discovery continues to succeed.
   - The smoke test currently marks `Database\Factories\ProductAnalyticsFactory` as incomplete because the referenced `App\Models\ProductAnalytics` class is missing. Reconcile the factory with a concrete model before relying on it for seeding workflows.
- Full database refreshes automatically run `Database\\Seeders\\AllSeedersSeeder`, which discovers and executes every seeder class in the tree. Reach for `php artisan db:seed --class=...` when a focused dataset keeps feedback loops manageable.【F:database/seeders/AllSeedersSeeder.php†L7-L109】【F:config/seeds.php†L28-L44】
  - The order item seeder now clamps Faker timestamp ranges so rerunning `db:seed` after factories create future-dated orders no longer trips "Start date must be anterior to end date" exceptions during `Database\Seeders\OrderItemSeeder`.【F:database/seeders/OrderItemSeeder.php†L29-L63】
...
- The comprehensive order seeder now inspects the persisted order numbers, primes an internal counter, and issues explicit identifiers during creation, which keeps rerunning the seeder from colliding with existing `ORD-######` records; reuse the helper when extending the dataset so numbering stays monotonic.【F:database/seeders/ComprehensiveOrderSeeder.php†L28-L122】
...
- Authorization policy coverage now centralises every policy class inside `tests/Unit/Policies/AuthorizationPolicyTest.php`. When introducing new permission strings or policy behaviours, extend the Gate helper definitions at the top of the file so `user->can()` checks simulate the intended ability matrix during unit runs.【F:tests/Unit/Policies/AuthorizationPolicyTest.php†L1-L472】
...
10. Global scopes now re-validate their cached schema metadata after migrations complete, so if a model suddenly surfaces unexpected rows (for example, `CustomerGroup::enabled()` returning disabled fixtures) rerun the test after the migration phase to let the refreshed cache take effect rather than patching around stale `is_active`/`is_enabled` filters.
    ...
    - Partner tier factories now append a ULID segment to the slug-based code, removing the 1,000-attempt ceiling that Faker's unique numerify previously imposed during large seeding runs. Preserve the ULID suffix when overriding factories so regression suites and seeders stay collision-free.
