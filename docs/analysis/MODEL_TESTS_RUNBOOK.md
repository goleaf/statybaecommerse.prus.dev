...
8. Capture any non-obvious insights or new invariants in the relevant markdown audits inside `docs/analysis/` so future tasks have the context baked in.
   ...
   - Menu authoring coverage now lives in `tests/Feature/Filament/Resources/MenuResourceTest.php` and `MenuItemResourceTest.php`; both suites call `resolveAdminPanel()` and `loadTable()` before asserting on Filament v4 tables, so mirror that structure when extending menu behaviour.【F:tests/Feature/Filament/Resources/MenuResourceTest.php†L1-L167】【F:tests/Feature/Filament/Resources/MenuItemResourceTest.php†L1-L152】
