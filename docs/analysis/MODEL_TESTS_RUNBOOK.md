...
8. Capture any non-obvious insights or new invariants in the relevant markdown audits inside `docs/analysis/` so future tasks have the context baked in.
   ...
   - Filament shims for resizable columns and legacy Livewire helpers are now covered by dedicated harnesses in `tests/Filament/HasResizableColumnsTest.php` and `tests/Filament/BaseListRecordsTest.php`. Maintain the guard-aware session key logic in `App\Filament\Concerns\HasResizableColumns` and the `BaseListRecords::loadTable()` / `create()` conveniences so the regression suite continues to validate these compatibility layers.【F:tests/Filament/HasResizableColumnsTest.php†L1-L124】【F:tests/Filament/BaseListRecordsTest.php†L1-L126】
