...
7. Capture any non-obvious insights or new invariants in the relevant markdown audits inside `docs/analysis/` so future tasks have the context baked in.
   ...
   - The shared `BaseListRecords::loadTable()` helper now bootstraps the Filament table instance when called before Livewire finishes mounting the component. Call `loadTable()` before asserting on columns or filters in feature tests so the v3-style helpers trigger table hydration consistently across list pages.
