...
8. Capture any non-obvious insights or new invariants in the relevant markdown audits inside `docs/analysis/` so future tasks have the context baked in.
   ...
   - Recommendation configuration combobox helpers rely on `tests/Filament/RecommendationConfigResourceSimpleHelperTest.php` to verify `normaliseRelationIdentifiers()` continues sorting, de-duplicating, and string-casting pivot identifiers for Filament v4 schemas. Mirror that behaviour when altering combobox hydration hooks so Livewire form state stays deterministic.【F:tests/Filament/RecommendationConfigResourceSimpleHelperTest.php†L1-L44】【F:app/Filament/Resources/RecommendationConfigResourceSimple.php†L294-L325】
