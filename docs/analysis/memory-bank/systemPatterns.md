...
- **Category tree API guardrail:** The `/api/categories/tree` endpoint now delegates its reserved slug list to `Category::RESERVED_SLUGS`, and the shared `Category::apiRouteBindingPattern()` regex keeps the literal `tree` slug (in any casing) mapped to the tree payload so implicit model binding never intercepts the request.【F:app/Models/Category.php†L46-L82】【F:routes/api.php†L74-L91】
