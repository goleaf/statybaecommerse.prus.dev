...
- **Category tree API guardrail:** The `/api/categories/tree` endpoint now applies a route constraint so the literal `tree` slug bypasses implicit model binding and always resolves to the cached tree response, preventing expensive 404 lookups when the catalogue is under load.【F:routes/api.php†L68-L85】
