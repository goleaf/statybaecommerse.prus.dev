...
On the PHP side, reuse the shared `App\Support\Filament\SearchableInputHelper` so hydration and clearing logic stays centralised. The helper’s `hydrate()` routine resolves an identifier into the canonical `[value, label, payload]` tuple, pushes that state back into the Livewire component, and normalises any arrayable payloads so downstream fields can consume them without extra casting. Conversely, the `clear()` helper accepts the `SearchableInput` instance alongside the form’s `Set` callback, resetting component state/options/payload before flushing any dependent fields you pass in, which prevents stale metadata from leaking into subsequent requests.【F:app/Support/Filament/SearchableInputHelper.php†L11-L86】 See the [Searchable input helper usage](../filament/searchable-inputs.md) note—especially the [resource integration checklist](../filament/searchable-inputs.md#resource-integration-checklist)—for full examples and the expected normaliser contract.
