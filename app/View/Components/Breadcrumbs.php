...
final class Breadcrumbs extends Component
{
    ...
    {
        return collect([['label' => __('frontend.navigation.home'), 'url' => url('/' . $this->locale)]])
            ->merge($this->items)
    ...
    private function buildLdItems(): array
    {
        $ldItems = [];
        $pos = 1;
        $trail = array_merge([['label' => __('frontend.navigation.home'), 'url' => url('/' . $this->locale)]], $this->items ?? []);
