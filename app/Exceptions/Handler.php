...
use App\Support\Exceptions\ActionableMessageGenerator;
use App\Support\Exceptions\BootErrorContextBuilder;
use App\Support\Exceptions\BootErrorDetector;
use App\Support\Exceptions\BootErrorLogger;
use App\Support\Exceptions\BootErrorMetricsTracker;
...
class Handler extends ExceptionHandler
{
    ...
    private ?BootErrorDetector $bootErrorDetector = null;

    private ?BootErrorLogger $bootErrorLogger = null;

    private ?BootErrorMetricsTracker $metricsTracker = null;
    ...
    {
        return $this->bootErrorDetector ??= new BootErrorDetector;
    }

    private function getBootErrorLogger(): BootErrorLogger
    {
        if ($this->bootErrorLogger === null) {
            $sanitizer = new ErrorMessageSanitizer;
            $messageGenerator = new ActionableMessageGenerator($sanitizer);
            $contextBuilder = new BootErrorContextBuilder(
                $this->getBootErrorDetector(),
                $sanitizer,
                $messageGenerator
            );
            $rateLimiter = new BootErrorRateLimiter;

            $this->bootErrorLogger = new BootErrorLogger($rateLimiter, $contextBuilder);
        }

        return $this->bootErrorLogger;
    }

    private function getMetricsTracker(): BootErrorMetricsTracker
    {
        return $this->metricsTracker ??= new BootErrorMetricsTracker($this->getBootErrorDetector());
    }
    ...
    {
        BootErrorDetector::resetCache();
        ErrorMessageSanitizer::resetCache();
