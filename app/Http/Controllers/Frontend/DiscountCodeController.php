...
class DiscountCodeController extends Controller
{
    ...
    public function validate(DiscountCodeValidationData $data): JsonResponse
    {
        ...
        if ($code->usage_limit_per_user && Auth::check()) {
            ...
            if ($userUsage >= $code->usage_limit_per_user) {
                return response()->json([
                    'valid' => false,
                    'message' => __('discount_code_already_used'),
                ], 422);
            }
        }
        ...
        return response()->json([
            ...
            'discount' => [
                ...
                'type' => $code->discount->type,
                'value' => $code->discount->value,
    ...
    /**
     * Apply a discount code to cart/order
     */
    public function apply(Request $request): JsonResponse
    {
        ...
        try {
            ...
            $redemption = DiscountRedemption::create([
                ...
                'user_id' => Auth::id(),
                'amount_saved' => 0, // Will be calculated based on order
                'currency_code' => 'EUR',
                'redeemed_at' => now(),
            ]);
    ...
    public function remove(Request $request): JsonResponse
    {
        ...
        $redemption = DiscountRedemption::where('code_id', $code->id)
            ->where('user_id', Auth::id())
            ->latest()
            ->first();
        ...
        return response()->json([
            'success' => true,
            'message' => __('discount_code_removed'),
        ]);
    }
    ...
    /**
     * Generate document for discount code
     */
    public function generateDocument(Request $request, DiscountCode $discountCode): Response
    {
        ...
        try {
            $document = $this->documentService->generateDocument(
                ...
                documentable: $discountCode,
                variables: [
                    'DISCOUNT_CODE' => $discountCode->code,
                    'DISCOUNT_NAME' => $discountCode->discount->name,
                    'DISCOUNT_DESCRIPTION' => $discountCode->description,
                    'DISCOUNT_VALUE' => $discountCode->discount->value,
                    'DISCOUNT_TYPE' => $discountCode->discount->type,
                    ...
                    'STARTS_AT' => $discountCode->starts_at?->format('d/m/Y H:i') ?? 'Immediately',
                    'EXPIRES_AT' => $discountCode->expires_at?->format('d/m/Y H:i') ?? 'Never',
