...
final class CampaignConversionController extends Controller
{
    ...
    {
        $query = CampaignConversion::with(['campaign', 'customer', 'order'])->orderBy('converted_at', 'desc');
        // Apply filters
    ...
    public function export(Request $request): \Symfony\Component\HttpFoundation\StreamedResponse
    {
        ...
        return response()->stream(function () use ($conversions) {
            ...
            foreach ($conversions as $conversion) {
                fputcsv($handle, [$conversion->id, $conversion->campaign?->name, $conversion->customer?->email, $conversion->order?->id, $conversion->conversion_type, $conversion->conversion_value, $conversion->status, $conversion->source, $conversion->medium, $conversion->device_type, $conversion->country, $conversion->city, $conversion->roi, $conversion->roas, $conversion->converted_at?->format('Y-m-d H:i:s')]);
            }
            fclose($handle);
        }, 200, $headers);
    }
