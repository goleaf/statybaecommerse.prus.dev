...
final class ReferralController extends Controller
{
    ...
    {
        $user = Auth::user();
        $validator = Validator::make($request->all(), ['referred_email' => 'required|email|exists:users,email', 'title' => 'nullable|string|max:255', 'description' => 'nullable|string|max:1000']);
        if ($validator->fails()) {
    ...
    public function generateCode(): JsonResponse
    {
        ...
        if ($user->referral_code) {
            return response()->json(['success' => false, 'message' => __('referrals.code_already_exists'), 'code' => $user->referral_code]);
        }
        try {
    ...
    {
        $validator = Validator::make($request->all(), ['code' => 'required|string|max:20']);
        if ($validator->fails()) {
        ...
        if (Referral::userAlreadyReferred($user->id)) {
            return response()->json(['success' => false, 'message' => __('referrals.user_already_referred')]);
        }
        ...
        if ($referral->referrer_id === $user->id) {
            return response()->json(['success' => false, 'message' => __('referrals.cannot_use_own_code')]);
        }
        try {
            DB::beginTransaction();
    ...
    {
        $user = Auth::user();
        $rewards = ReferralReward::where('user_id', $user->id)->with(['referral.referred'])->orderBy('created_at', 'desc')->paginate(10);
        $totalRewards = ReferralReward::where('user_id', $user->id)->sum('amount');
        $pendingRewards = ReferralReward::where('user_id', $user->id)->pending()->sum('amount');
        $appliedRewards = ReferralReward::where('user_id', $user->id)->applied()->sum('amount');
