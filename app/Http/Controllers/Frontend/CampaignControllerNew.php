...
final class CampaignControllerNew extends Controller
{
    ...
    public function click(Campaign $campaign, Request $request): JsonResponse
    {
        ...
        $validated = $request->validate([
            'click_type' => 'string|in:cta,banner,link',
            'clicked_url' => 'nullable|url',
        ]);

        $campaign->recordClick(
            clickType: $validated['click_type'] ?? 'cta',
            clickedUrl: $validated['clicked_url'] ?? $campaign->cta_url,
            sessionId: session()->getId(),
            ipAddress: request()->ip(),
            userAgent: request()->userAgent(),
            customerId: auth()->id()
        ...
        return response()->json([
            'success' => true,
            'message' => __('campaigns.frontend.click_recorded'),
            'campaign' => [
                'id' => $campaign->id,
                'name' => $campaign->name,
    ...
    public function conversion(Campaign $campaign, Request $request): JsonResponse
    {
        ...
        $validated = $request->validate([
            'conversion_type' => 'string|in:purchase,signup,download,subscription',
        ...
        $campaign->recordConversion(
            conversionType: $validated['conversion_type'] ?? 'purchase',
            conversionValue: $validated['conversion_value'] ?? 0,
            orderId: $validated['order_id'] ?? null,
            customerId: auth()->id(),
            sessionId: session()->getId(),
            conversionData: $validated['conversion_data'] ?? []
        );

        return response()->json([
            'success' => true,
            'message' => __('campaigns.frontend.conversion_recorded'),
            'campaign' => [
                'id' => $campaign->id,
                'name' => $campaign->name,
        ...
        $campaigns = Campaign::active()
            ...
            ->map(function ($campaign) {
                return [
                    ...
                    'image_url' => $campaign->getImageUrl('medium'),
                    'banner_url' => $campaign->getBannerUrl('medium'),
                ];
            });
