...
final class ProductHistoryController extends Controller
{
    ...
    public function statistics(Request $request, Product $product): JsonResponse|View|Response
    {
        ...
        $changesByAction = $product->histories()
            ->selectRaw('action, COUNT(*) as count')
            ->groupBy('action')
            ->pluck('count', 'action');

        $changesByField = $product->histories()
            ->selectRaw('field_name, COUNT(*) as count')
            ->whereNotNull('field_name')
            ->groupBy('field_name')
            ->pluck('count', 'field_name');

        $recentActivity = $product->histories()
            ->with(['user:id,name'])
            ->recent(7)
            ->limit(5)
    ...
    public function export(Request $request, Product $product): Response
    {
        ...
        foreach ($histories as $history) {
            $csvData .= sprintf(
                ...
                $history->action,
                $history->field_name ?? 'N/A',
                is_array($history->old_value) ? json_encode($history->old_value) : ($history->old_value ?? 'N/A'),
                is_array($history->new_value) ? json_encode($history->new_value) : ($history->new_value ?? 'N/A'),
                str_replace(["\r", "\n"], ' ', $history->description ?? ''),
    ...
    public function create(Request $request, Product $product): JsonResponse
    {
        ...
        $history = ProductHistory::createHistoryEntry(
            ...
            oldValue: $request->get('old_value'),
