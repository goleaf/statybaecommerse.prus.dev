...
final class CollectionController extends Controller
{
    ...
    public function index(Request $request): View
    {
        $query = Collection::withTranslations()
            ...
            ->ordered()
            ->with([
                'products' => function ($productQuery): void {
                    $productQuery
                        ->published()
            ...
            ->withCount([
                'products as published_products_count' => function ($countQuery): void {
    ...
    public function show(Collection $collection): View
    {
        ...
        if (! $collection->relationLoaded('products.images') || ! $collection->relationLoaded('products.translations')) {
            $collection->load(['products' => function ($query): void {
                $query->published()->with(['images', 'translations']);
            }]);
        }
        $relatedCollections = Collection::withTranslations()->visible()->where('id', '!=', $collection->id)->where('display_type', $collection->display_type)->limit(4)->get()->skipWhile(fn ($relatedCollection): bool => // Skip related collections that are not properly configured
            empty($relatedCollection->name) || ! $relatedCollection->is_visible || empty($relatedCollection->slug) || $relatedCollection->products()->count() <= 0);
        ...
        return view('collections.show', ['collection' => $collection, 'relatedCollections' => $relatedCollections, 'organizedProducts' => $organizedProducts, 'products' => $products]);
    }
    ...
    public function api(Request $request): JsonResponse
    {
        ...
        return response()->json(['data' => $collections->skipWhile(fn ($collection): bool => // Skip collections that are not properly configured or have missing essential data
            empty($collection->name) || ! $collection->is_visible || empty($collection->slug))->map(fn ($collection): array => [
                'id'             => $collection->id,
    ...
    public function withProducts(): JsonResponse
    {
        ...
        return response()->json(['data' => $collections->skipWhile(fn ($collection): bool => // Skip collections that are not properly configured or have missing essential data
                    empty($collection->name) || ! $collection->is_visible || empty($collection->slug) || $collection->products_count <= 0)->map(fn ($collection): array => [
                        'id'             => $collection->id,
                        'name'           => $collection->getTranslatedName(),
                        'slug'           => $collection->slug,
                        'description'    => $collection->getTranslatedDescription(),
    ...
    public function products(Collection $collection): JsonResponse
    {
        ...
        $products = PaginationService::paginateQueryWithSkipWhile($collection->products()->published()->with(['images', 'translations'])->getQuery(), fn ($product): bool => // Skip products that are not properly configured for display
            empty($product->name) || ! $product->is_visible || $product->price <= 0 || empty($product->slug) || ! $product->getFirstMediaUrl('images'), $collection->products_per_page ?: 12, 2);

        $data = collect($products->items())->map(fn ($product): array => [
            'id'          => $product->id,
            'name'        => $product->name,
            'slug'        => $product->slug,
    ...
    public function productsGallery(Collection $collection, Request $request): JsonResponse
    {
        ...
        if (array_filter($filters) !== []) {
            ...
            $products = $products->skipWhile(fn ($product): bool => empty($product->name) || ! $product->is_visible || $product->price <= 0 || empty($product->slug) || ! $product->getFirstMediaUrl('images'));
        }
    ...
    public function homepageLayout(Request $request): JsonResponse
    {
        ...
        $collections = Collection::withTranslations()->visible()->withCount('products')->orderBy('sort_order')->get()->skipWhile(fn ($collection): bool => // Skip collections that are not suitable for homepage display
            empty($collection->name) || ! $collection->is_visible || empty($collection->slug) || $collection->products_count <= 0 || ! $collection->getImageUrl('sm'));
        $organizedCollections = $collections->splitIn($columnCount);

        return response()->json(['layout' => ['columns' => $columnCount, 'total_collections' => $collections->count(), 'columns_data' => $organizedCollections->map(fn ($columnCollections, $columnIndex): array => ['column_id' => $columnIndex + 1, 'item_count' => $columnCollections->count(), 'collections' => $columnCollections->map(fn ($collection): array => ['id' => $collection->id, 'name' => $collection->getTranslatedName(), 'slug' => $collection->slug, 'description' => $collection->getTranslatedDescription(), 'image_url' => $collection->getImageUrl('sm'), 'products_count' => $collection->products_count, 'display_type' => $collection->display_type])])]]);
    }
    ...
    public function newArrivals(Request $request): JsonResponse
    {
        ...
        $columnCount = $request->get('columns', 4);
        $collections = Collection::withTranslations()->visible()->withCount('products')->orderBy('sort_order')->get()->skipWhile(fn ($collection): bool => empty($collection->name) || ! $collection->is_visible || empty($collection->slug) || $collection->products_count <= 0);
        $galleryService = new ProductGalleryService;
        $allProducts = collect();
        ...
        LazyCollection::make($collections)->takeUntilTimeout($timeout)->each(function ($collection) use (&$allProducts, $galleryService, $days): void {
            $products = $collection->products()->published()->with(['images', 'translations'])->get();
        ...
        $uniqueProducts = $allProducts->unique('id')->skipWhile(fn ($product): bool => empty($product->name) || ! $product->is_visible || $product->price <= 0 || empty($product->slug));
