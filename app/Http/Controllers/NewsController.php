...
final class NewsController extends Controller
{
    ...
    public function index(Request $request): View|JsonResponse
    {
        ...
        $featuredNews = News::published()
            ...
            ->limit(3)
            ->get()
            ->filter(static function (News $newsItem): bool {
                // Guard against entries lacking translated metadata or a highlighted image so the featured rail never renders broken tiles.
                $hasFeaturedImage = $newsItem->images->where('is_featured', true)->first()?->url !== null;
            ...
            ->values(); // Reindex the collection to keep Blade loops predictable after filtering.
    ...
    public function show(string $slug): View
    {
        ...
        $relatedNews = News::published()
            ...
            ->limit(4)
            ->get()
            ->filter(static function (News $candidate): bool {
                // Ensure every related card exposes localized metadata and a featured image before surfacing it alongside the article.
                $hasFeaturedImage = $candidate->images->where('is_featured', true)->first()?->url !== null;
