...
{
    /**
     * Generate sales report with timeout protection
     */
    public function generateSalesReport(array $filters = []): array
    {
        $timeout = now()->addMinutes(5); // 5 minute timeout for sales report generation
        
        $query = AnalyticsEvent::where('event_type', 'purchase')
            ->with(['user', 'trackable'])
            ->whereNotNull('value');
        ...
        $query->cursor()
            ...
            ->each(function ($event) use (&$salesData, &$totalRevenue, &$processedCount) {
                $processedCount++;
    ...
    /**
     * Generate product analytics report with timeout protection
     */
    public function generateProductAnalyticsReport(array $filters = []): array
    {
        $timeout = now()->addMinutes(10); // 10 minute timeout for product analytics
        
        $query = Product::with(['categories', 'brand', 'media'])
        ...
        $query->cursor()
            ->takeUntilTimeout($timeout)
            ->each(function ($product) use (&$productData, &$processedCount) {
                $processedCount++;
        ...
        Log::info('Product analytics report generated', [
    ...
    /**
     * Generate user activity report with timeout protection
     */
    public function generateUserActivityReport(array $filters = []): array
    {
        $timeout = now()->addMinutes(8); // 8 minute timeout for user activity report
        ...
        $query->cursor()
            ->takeUntilTimeout($timeout)
            ->each(function ($event) use (&$userActivity, &$processedCount) {
                $processedCount++;
                ...
                if (!$userActivity[$userId]['last_activity'] || 
                    $event->created_at->greaterThan($userActivity[$userId]['last_activity'])) {
                    $userActivity[$userId]['last_activity'] = $event->created_at->format('Y-m-d H:i:s');
                }
            });

        Log::info('User activity report generated', [
            'processed_events' => $processedCount,
        ...
        return [
            ...
            'user_activity' => array_values($userActivity),
        ];
    }

    /**
     * Generate comprehensive system report with timeout protection
     */
    public function generateSystemReport(): array
    {
        $timeout = now()->addMinutes(15); // 15 minute timeout for comprehensive system report
        ...
        $sections = [
            'users' => fn() => $this->generateUserStats(),
            'products' => fn() => $this->generateProductStats(),
            'analytics' => fn() => $this->generateAnalyticsStats(),
        ];

        foreach ($sections as $sectionName => $sectionGenerator) {
            if (now()->greaterThan($timeout)) {
                Log::warning('System report generation timeout reached', [
            ...
            try {
                ...
                Log::error("Failed to generate {$sectionName} section", [
                    'error' => $e->getMessage(),
                ]);
    ...
    private function generateProductStats(): array
    {
        ...
        Product::cursor()
            ->takeUntilTimeout($timeout)
            ->each(function ($product) use (&$stats) {
                $stats['total']++;
                if ($product->is_visible) $stats['visible']++;
                if ($product->is_featured) $stats['featured']++;
                if ($product->stock_quantity > 0) $stats['with_stock']++;
            });
