...
final class SystemSettingsService
{
    ...
    public function bulkUpdate(array $settings): void
    {
        try {
            ...
            foreach ($settings as $key => $value) {
                SystemSetting::where('key', $key)->update(['value' => $value, 'updated_by' => auth()->id()]);
            }
            $this->clearCache();
            DB::commit();
    ...
    public function resetToDefaults(): void
    {
        try {
            ...
            foreach ($settings as $setting) {
                $setting->update(['value' => $setting->default_value, 'updated_by' => auth()->id()]);
            }
            $this->clearCache();
            DB::commit();
    ...
    {
        return SystemSetting::active()->get()->map(function ($setting) {
        ...
        try {
            ...
            foreach ($settings as $settingData) {
                if (isset($settingData['key'])) {
                    SystemSetting::updateOrCreate(['key' => $settingData['key']], array_merge($settingData, ['updated_by' => auth()->id()]));
                }
            }
            $this->clearCache();
            DB::commit();
    ...
    {
        return ['total' => SystemSetting::count(), 'active' => SystemSetting::active()->count(), 'public' => SystemSetting::public()->count(), 'encrypted' => SystemSetting::where('is_encrypted', true)->count(), 'categories' => SystemSettingCategory::active()->count(), 'by_type' => SystemSetting::selectRaw('type, count(*) as count')->active()->groupBy('type')->pluck('count', 'type')->toArray(), 'by_group' => SystemSetting::selectRaw('group, count(*) as count')->active()->groupBy('group')->pluck('count', 'group')->toArray()];
    }
}