...
final class SEOService
{
    ...
    public static function getProductSEO(Product $product): array
    {
        ...
        return ['title' => $title, 'description' => $description, 'keywords' => $keywords, 'canonical' => route('product.show', $product->slug), 'og_title' => $title, 'og_description' => $description, 'og_image' => $product->getFirstMediaUrl('images', 'image-lg') ?: $product->getFirstMediaUrl('images'), 'og_type' => 'product', 'product_price' => number_format((float) $product->price, 2), 'product_currency' => 'EUR', 'product_availability' => $product->stock_quantity > 0 ? 'in stock' : 'out of stock'];
    }
    ...
    {
        $keywords = collect([$product->name, $product->brand?->name, $product->categories->first()?->name, $product->sku])->filter()->map(fn ($keyword) => Str::slug($keyword, ' '))->implode(', ');
    ...
    {
        return ['@context' => 'https://schema.org/', '@type' => 'Product', 'name' => $product->name, 'description' => strip_tags($product->description), 'sku' => $product->sku, 'image' => $product->getFirstMediaUrl('images', 'image-lg') ?: $product->getFirstMediaUrl('images'), 'brand' => ['@type' => 'Brand', 'name' => $product->brand?->name ?? config('app.name')], 'offers' => ['@type' => 'Offer', 'price' => number_format((float) $product->price, 2), 'priceCurrency' => 'EUR', 'availability' => $product->stock_quantity > 0 ? 'https://schema.org/InStock' : 'https://schema.org/OutOfStock', 'seller' => ['@type' => 'Organization', 'name' => config('app.name')]]];
    }
}
