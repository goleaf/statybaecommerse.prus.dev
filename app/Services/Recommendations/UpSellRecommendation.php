...
final class UpSellRecommendation extends BaseRecommendation
{
    protected function getDefaultConfig(): array
    {
        return [
            'max_results' => 10,
            'min_score' => 0.1,
            'min_price_increase' => 1.1, // At least 10% more expensive
            'max_price_increase' => 2.0, // At most 2x more expensive
    ...
    private function calculateUpSellRecommendations(Product $product, ?User $user = null): Collection
    {
        ...
        $query = Product::query()
            ->with(['media', 'brand', 'categories', 'reviews'])
    ...
    private function calculateUpSellScores(Product $product, Collection $candidateProducts): Collection
    {
        ...
        foreach ($candidateProducts as $candidateProduct) {
            ...
            $totalScore = (
                ...
                $priceScore * $this->config['price_ratio_weight'] +
                $qualityScore * $this->config['quality_indicators_weight']
            );
            ...
            if ($totalScore >= $this->minScore) {
                $scores->push([
                    ...
                    'price_increase_ratio' => $candidateProduct->price / $product->price,
                ]);
            }
        }
    ...
    private function calculateQualityIndicatorScore(Product $product): float
    {
        ...
        if (isset($indicators['review_count'])) {
            ...
            $reviewScore = min($reviewCount / 50.0, 1.0); // Cap at 1.0 for 50+ reviews
        ...
        if (isset($indicators['sales_count'])) {
            $salesCount = $product->orderItems()
                ...
                ->sum('quantity');
            $salesScore = min($salesCount / 100.0, 1.0); // Cap at 1.0 for 100+ sales
    ...
    public function getUpSellOpportunities(User $user): Collection
    {
        ...
        foreach ($purchasedProducts as $purchasedProduct) {
            ...
            if ($upSellRecommendations->isNotEmpty()) {
                $opportunities->push([
                    'original_product' => $purchasedProduct,
                    'up_sell_products' => $upSellRecommendations,
                    'potential_revenue_increase' => $this->calculatePotentialRevenueIncrease(
                        $purchasedProduct,
    ...
    public function getUpSellAnalytics(Product $product): array
    {
        ...
        $upSellCandidates = Product::where('is_visible', true)
            ->where('id', '!=', $product->id)
            ->whereBetween('price', [$minPrice, $maxPrice])
        ...
        return [
            'total_candidates' => $upSellCandidates->count(),
            'price_range' => [
                'min' => $minPrice,
                'max' => $maxPrice,
                'avg' => $upSellCandidates->avg('price'),
            ],
            'category_distribution' => $this->getUpSellCategoryDistribution($upSellCandidates),
            'quality_distribution' => $this->getUpSellQualityDistribution($upSellCandidates),
            'conversion_potential' => $this->calculateConversionPotential($product, $upSellCandidates),
        ];
    }

    private function getUpSellCategoryDistribution(Collection $products): Collection
    {
        return $products->pluck('categories')
            ->flatten()
            ->groupBy('name')
    ...
    {
        return [
            'avg_rating' => $products->avg(function ($product) {
