...
final class PopularityRecommendation extends BaseRecommendation
{
    ...
    private function calculatePopularityRecommendations(?User $user = null, ?Product $product = null): Collection
    {
        ...
        $query->leftJoinSub(
            $this->getSalesSubquery($timeWindow),
            'sales_stats',
            'products.id',
            '=',
        ...
        $query->leftJoinSub(
            $this->getViewsSubquery($timeWindow),
            'view_stats',
            'products.id',
            '=',
            'view_stats.product_id'
        );

        $query->leftJoinSub(
            $this->getReviewsSubquery($timeWindow),
            'review_stats',
            'products.id',
            '=',
        ...
        $query->leftJoinSub(
            $this->getWishlistSubquery($timeWindow),
            'wishlist_stats',
            'products.id',
            '=',
            'wishlist_stats.product_id'
        );
    ...
    private function getViewsSubquery(int $timeWindow): \Illuminate\Database\Eloquent\Builder
    {
        ...
        return Product::query()
            ->selectRaw('id as product_id, COALESCE(view_count, 0) as view_count')
            ->where('created_at', '>=', now()->subDays($timeWindow));
    }
    ...
    private function getWishlistSubquery(int $timeWindow): \Illuminate\Database\Eloquent\Builder
    {
        ...
        return Product::query()
            ->selectRaw('id as product_id, 0 as wishlist_count')
