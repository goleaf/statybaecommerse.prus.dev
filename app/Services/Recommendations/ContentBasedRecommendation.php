...
final class ContentBasedRecommendation extends BaseRecommendation
{
    ...
    private function calculateContentBasedRecommendations(Product $product, ?User $user = null): Collection
    {
        ...
        if ($this->config['use_cached_similarities']) {
            $cachedSimilarities = ProductSimilarity::where('product_id', $product->id)
                ->byAlgorithm('content_based')
                ->recent($this->config['recalculate_threshold'])
                ->withMinScore($this->minScore)
                ->orderedBySimilarity()
                ->with('similarProduct')
            ...
            if ($cachedSimilarities->isNotEmpty()) {
                return $cachedSimilarities->pluck('similarProduct')
                    ->filter(fn($p) => $p && $p->is_visible);
            }
        }
    ...
    private function cacheSimilarities(int $productId, Collection $similarities): void
    {
        ...
        foreach ($similarities as $index => $similarProduct) {
            $similarityData[] = [
                ...
                'similarity_score' => 1.0 - ($index * 0.1), // Approximate score based on position
                'calculation_data' => [
                    'calculated_at' => now(),
        ...
        if (!empty($similarityData)) {
            DB::table('product_similarities')->upsert(
                $similarityData,
                ['product_id', 'similar_product_id', 'algorithm_type'],
                ['similarity_score', 'calculation_data', 'calculated_at', 'updated_at']
            );
        }
    }
}
