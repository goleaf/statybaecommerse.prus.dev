...
final class TrendingRecommendation extends BaseRecommendation
{
    ...
    public function getRecommendations(
        ...
        return $this->cacheResult($cacheKey, $recommendations, $this->config['cache_ttl'] ?? 900); // 15 minutes
    }
    ...
    private function calculateTrendingRecommendations(?User $user = null, ?Product $product = null): Collection
    {
        ...
        ', [
            now()->subDays($this->config['new_product_threshold']),
            $weights['recent_sales'],
            $weights['recent_views'],
            $weights['recent_reviews'],
            $weights['recent_wishlist'],
            now()->subDays($this->config['new_product_threshold']),
        ]);

        // Add subqueries for recent activity
        $query->leftJoinSub(
            $this->getRecentSalesSubquery($timeWindow),
            'recent_sales_stats',
            'products.id',
            '=',
            'recent_sales_stats.product_id'
        );

        $query->leftJoinSub(
            $this->getRecentViewsSubquery($timeWindow),
            'recent_view_stats',
            'products.id',
            '=',
            'recent_view_stats.product_id'
        );

        $query->leftJoinSub(
            $this->getRecentReviewsSubquery($timeWindow),
            'recent_review_stats',
            'products.id',
            '=',
            'recent_review_stats.product_id'
        );

        $query->leftJoinSub(
            $this->getRecentWishlistSubquery($timeWindow),
            'recent_wishlist_stats',
            'products.id',
            '=',
            'recent_wishlist_stats.product_id'
        );

        // Add subqueries for previous period activity (for velocity calculation)
        $query->leftJoinSub(
            $this->getPreviousSalesSubquery($timeWindow),
            'previous_sales_stats',
            'products.id',
            '=',
            'previous_sales_stats.product_id'
        );

        $query->leftJoinSub(
            $this->getPreviousViewsSubquery($timeWindow),
            'previous_view_stats',
            'products.id',
            '=',
            'previous_view_stats.product_id'
        );

        $query->leftJoinSub(
            $this->getPreviousReviewsSubquery($timeWindow),
            'previous_review_stats',
            'products.id',
            '=',
            'previous_review_stats.product_id'
        );

        $query->leftJoinSub(
            $this->getPreviousWishlistSubquery($timeWindow),
            'previous_wishlist_stats',
            'products.id',
            '=',
            'previous_wishlist_stats.product_id'
        );
    ...
    private function getRecentSalesSubquery(int $timeWindow): \Illuminate\Database\Eloquent\Builder
    {
        return \App\Models\OrderItem::query()
            ->selectRaw('product_id, COUNT(*) as recent_sales')
            ->whereHas('order', function ($query) use ($timeWindow) {
                $query->whereIn('status', ['completed', 'delivered'])
                    ->where('created_at', '>=', now()->subDays($timeWindow));
            })
            ->groupBy('product_id');
    }
    ...
    private function getRecentViewsSubquery(int $timeWindow): \Illuminate\Database\Eloquent\Builder
    {
        if (class_exists(\App\Models\UserBehavior::class)) {
            return \App\Models\UserBehavior::query()
        ...
        return Product::query()
            ->selectRaw('id as product_id, 0 as recent_views')
            ->whereRaw('1 = 0');
    }
    ...
    private function getRecentReviewsSubquery(int $timeWindow): \Illuminate\Database\Eloquent\Builder
    {
        return \App\Models\Review::query()
            ->selectRaw('product_id, COUNT(*) as recent_reviews')
            ->where('created_at', '>=', now()->subDays($timeWindow))
            ->groupBy('product_id');
    }
    ...
    private function getRecentWishlistSubquery(int $timeWindow): \Illuminate\Database\Eloquent\Builder
    {
        if (class_exists(\App\Models\UserBehavior::class)) {
            return \App\Models\UserBehavior::query()
                ->selectRaw('product_id, COUNT(*) as recent_wishlist')
                ->where('behavior_type', 'wishlist')
                ->where('created_at', '>=', now()->subDays($timeWindow))
                ->groupBy('product_id');
        }

        return Product::query()
            ->selectRaw('id as product_id, 0 as recent_wishlist')
            ->whereRaw('1 = 0');
    }
    ...
    {
        return \App\Models\OrderItem::query()
            ->selectRaw('product_id, COUNT(*) as previous_sales')
            ->whereHas('order', function ($query) use ($timeWindow) {
                $query->whereIn('status', ['completed', 'delivered'])
                    ->whereBetween('created_at', [
                        now()->subDays($timeWindow * 2),
                        now()->subDays($timeWindow)
                    ]);
            })
            ->groupBy('product_id');
    }
    ...
    private function getPreviousViewsSubquery(int $timeWindow): \Illuminate\Database\Eloquent\Builder
    {
        ...
        return Product::query()
            ->selectRaw('id as product_id, 0 as previous_views')
            ->whereRaw('1 = 0');
    }
    ...
    {
        return \App\Models\Review::query()
            ->selectRaw('product_id, COUNT(*) as previous_reviews')
            ->whereBetween('created_at', [
                now()->subDays($timeWindow * 2),
                now()->subDays($timeWindow)
            ])
    ...
    private function getPreviousWishlistSubquery(int $timeWindow): \Illuminate\Database\Eloquent\Builder
    {
        if (class_exists(\App\Models\UserBehavior::class)) {
            return \App\Models\UserBehavior::query()
                ->selectRaw('product_id, COUNT(*) as previous_wishlist')
                ->where('behavior_type', 'wishlist')
                ->whereBetween('created_at', [
                    now()->subDays($timeWindow * 2),
                    now()->subDays($timeWindow)
        ...
        return Product::query()
            ->selectRaw('id as product_id, 0 as previous_wishlist')
            ->whereRaw('1 = 0');
    }
    ...
    public function getTrendingCategories(): Collection
    {
        ...
        return $trendingProducts->groupBy(function ($product) {
            ...
            return [
                ...
                'products' => $products,
                'trend_score' => $products->sum('trending_score'),
            ];
