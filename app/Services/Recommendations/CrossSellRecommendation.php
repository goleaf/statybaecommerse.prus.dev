...
final class CrossSellRecommendation extends BaseRecommendation
{
    protected function getDefaultConfig(): array
    {
        return [
            ...
            'max_price_ratio' => 2.0, // Max 2x the original price
            'min_price_ratio' => 0.5, // Min 0.5x the original price
        ];
    }
    ...
    private function getCoPurchasedProducts(Product $product): Collection
    {
        ...
        $orderIds = DB::table('order_items')
            ...
            ->where('orders.created_at', '>=', now()->subDays($timeWindow))
            ->pluck('orders.id');
        ...
        $coPurchasedData = DB::table('order_items')
            ->join('orders', 'order_items.order_id', '=', 'orders.id')
            ...
            ->having('co_purchase_count', '>=', $minCoPurchaseCount)
    ...
    private function calculateCrossSellScores(Product $product, Collection $coPurchasedProducts): Collection
    {
        ...
        foreach ($coPurchasedProducts as $coProduct) {
            ...
            $totalScore = (
                $coPurchaseScore * $this->config['co_purchase_weight'] +
                $categoryScore * $this->config['category_similarity_weight'] +
                $priceScore * $this->config['price_compatibility_weight']
            );
            ...
            if ($totalScore >= $this->minScore) {
                $scores->push([
                    ...
                    'co_purchase_score' => $coPurchaseScore,
    ...
    private function calculateCoPurchaseScore(int $coPurchaseCount, int $uniqueCustomers): float
    {
        ...
        $frequencyScore = min($coPurchaseCount / 10.0, 1.0); // Cap at 1.0 for 10+ co-purchases
        $diversityScore = min($uniqueCustomers / 5.0, 1.0); // Cap at 1.0 for 5+ unique customers
    ...
    private function getCategoryBasedCrossSell(Product $product): Collection
    {
        ...
        return Product::query()
            ...
            ->whereBetween('price', [
                $product->price * $this->config['min_price_ratio'],
                $product->price * $this->config['max_price_ratio']
            ])
            ->inRandomOrder()
        ...
        return DB::table('order_items as oi1')
            ->join('order_items as oi2', function ($join) {
                $join->on('oi1.order_id', '=', 'oi2.order_id')
                     ->on('oi1.product_id', '<', 'oi2.product_id'); // Avoid duplicates
            })
            ...
            ->selectRaw('
                ...
                COUNT(DISTINCT orders.user_id) as unique_customers
            ')
            ->groupBy('oi1.product_id', 'oi2.product_id', 'p1.name', 'p2.name')
            ->having('co_purchase_count', '>=', $this->config['min_co_purchase_count'])
            ->orderByDesc('co_purchase_count')
            ->limit($limit)
            ->get();
    }
    ...
    public function getProductCrossSellAnalytics(Product $product): array
    {
        ...
        return [
            'total_co_purchase_products' => $coPurchasedProducts->count(),
            'total_co_purchases' => $coPurchasedProducts->sum('co_purchase_count'),
            'unique_customers' => $coPurchasedProducts->sum('unique_customers'),
            'top_cross_sell_categories' => $this->getTopCrossSellCategories($product),
            'price_distribution' => $this->getCrossSellPriceDistribution($product, $coPurchasedProducts),
        ];
    }
