...
final class CollaborativeFilteringRecommendation extends BaseRecommendation
{
    ...
    {
        return UserProductInteraction::where('user_id', $user->id)
            ->withMinCount($this->config['min_interactions'])
            ->with('product')
            ->get()
            ->keyBy('product_id');
    }
    ...
    private function findSimilarUsers(User $user, Collection $userInteractions): Collection
    {
        ...
        $candidateUsers = UserProductInteraction::whereIn('product_id', $userProductIds)
        ...
        return $neighbors
            ...
            ->take($this->config['max_neighbors']);
    }
    ...
    private function generateRecommendationsFromNeighbors(
        User $user,
        ...
        foreach ($neighbors as $neighbor) {
            ...
            foreach ($neighborInteractions as $interaction) {
                ...
                $weightedScore = $interaction->rating * $similarity * 
                    ($this->config['interaction_weights'][$interaction->interaction_type] ?? 1.0);
        ...
        $productScores = $recommendations->groupBy('product_id')
            ->map(function ($group) {
                ...
                return [
                    ...
                    'score' => $totalScore / $interactionCount, // Average score
    ...
    public function updateUserInteraction(
        User $user,
        Product $product,
