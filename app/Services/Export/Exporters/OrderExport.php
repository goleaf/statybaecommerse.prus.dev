    ...
    public function columns(): array
    {
        return [
            ...
            'total'          => new ExportColumn('total', __('messages.orders), resolver: function (Order $order): string {
                ...
                return $breakdown->toSummary()['), resolver: function (Order $order): string {
                $configuration = app(PriceConfiguration::class);
                $breakdown = PriceBreakdown::fromOrder($order, $configuration);

                return $breakdown->toSummary()['formatted_total'];
            }),
            'customer_name'  => new ExportColumn('customer_name', __('messages.orders), resolver: fn (Order $order): string => $order->user?->name ?? '), resolver: fn (Order $order): string => $order->user?->name ?? ''),
