...
final class EmailMarketingService
{
    ...
    public function syncSubscriberToMailchimp(Subscriber $subscriber): bool
    {
        try {
            $response = Http::withHeaders([
                ...
                'status' => $this->mapStatusToMailchimp($subscriber->status),
                'merge_fields' => [
                ...
                'marketing_permissions' => [
                    [
                        ...
                        'enabled' => $subscriber->status === 'active',
                    ],
                ],
            ]);

            if ($response->successful()) {
                Log::info('Subscriber synced to Mailchimp', [
            ...
            Log::error('Failed to sync subscriber to Mailchimp', [
                'subscriber_id' => $subscriber->id,
            ...
            Log::error('Exception syncing subscriber to Mailchimp', [
                'subscriber_id' => $subscriber->id,
                'error' => $e->getMessage(),
            ]);
        ...
        try {
            $response = Http::withHeaders([
                ...
                'recipients' => [
                    'list_id' => $this->listId,
                    'segment_opts' => [
                        'saved_segment_id' => $campaignData['segment_id'] ?? null,
                    ],
                ],
                'settings' => [
                    'subject_line' => $campaignData['subject'],
                    'from_name' => $campaignData['from_name'] ?? config('app.name'),
                    'reply_to' => $campaignData['reply_to'] ?? config('mail.from.address'),
            ...
            if ($response->successful()) {
                $data = $response->json();
                Log::info('Campaign created in Mailchimp', [
                    'campaign_id' => $data['id'],
                    'title' => $campaignData['title'],
                ]);
                return $data['id'];
            ...
            Log::error('Failed to create campaign in Mailchimp', [
                'response' => $response->body(),
            ]);

            return null;
        } catch (\Exception $e) {
            Log::error('Exception creating campaign in Mailchimp', [
                'error' => $e->getMessage(),
            ]);
    ...
    /**
     * Get campaign analytics from Mailchimp
     */
    public function getCampaignAnalytics(string $campaignId): ?array
    {
        try {
            ...
            Log::error('Exception getting campaign analytics', [
    ...
    /**
     * Bulk sync subscribers to Mailchimp
     */
    ...
    /**
     * Get subscriber stats from Mailchimp
     */
    public function getMailchimpStats(): ?array
    {
        try {
            $response = Http::withHeaders([
                'Authorization' => 'Bearer ' . $this->apiKey,
            ])->get("{$this->baseUrl}/lists/{$this->listId}");

            if ($response->successful()) {
                $data = $response->json();
                return [
                    'total_subscribers' => $data['stats']['member_count'],
            ...
            Log::error('Exception getting Mailchimp stats', [
                'error' => $e->getMessage(),
            ]);
    ...
    /**
     * Map internal status to Mailchimp status
     */
    ...
    /**
     * Create segment in Mailchimp based on subscriber interests
     */
    public function createInterestSegment(string $interest): ?string
    {
        try {
            $response = Http::withHeaders([
                'Authorization' => 'Bearer ' . $this->apiKey,
                'Content-Type' => 'application/json',
            ])->post("{$this->baseUrl}/lists/{$this->listId}/segments", [
                'name' => ucfirst($interest) . ' Subscribers',
                'options' => [
                    'match' => 'any',
                    'conditions' => [
                        [
            ...
            if ($response->successful()) {
                $data = $response->json();
                Log::info('Interest segment created in Mailchimp', [
                    'segment_id' => $data['id'],
                    'interest' => $interest,
                ]);
                return $data['id'];
            ...
            Log::error('Exception creating interest segment', [
                'interest' => $interest,
                'error' => $e->getMessage(),
            ]);
