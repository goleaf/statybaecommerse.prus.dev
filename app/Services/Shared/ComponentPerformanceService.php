...
final class ComponentPerformanceService
{
    ...
    public function optimizeSlowComponents(): array
    {
        ...
        foreach ($slowComponents as $component => $metrics) {
            ...
            if ($avgTime > 200) {
                $recommendations[$component] = ['severity' => 'high', 'avg_time' => $avgTime, 'recommendations' => ['Consider caching component output', 'Optimize database queries', 'Reduce component complexity', 'Use lazy loading for heavy content']];
            } elseif ($avgTime > 100) {
                $recommendations[$component] = ['severity' => 'medium', 'avg_time' => $avgTime, 'recommendations' => ['Review component logic', 'Consider prop optimization', 'Check for unnecessary re-renders']];
            } elseif ($avgTime > 50) {
