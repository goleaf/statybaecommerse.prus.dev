...
final class RecommendationService
{
    ...
    public function getRecommendations(
        ...
        try {
            ...
            Log::error("Recommendation generation failed for block '{$blockName}'", [
                'error' => $e->getMessage(),
                'user_id' => $user?->id,
    ...
    private function generateCacheKey(
        ...
        return RecommendationCache::generateCacheKey(
            $block->name,
            $user?->id,
            $product?->id,
            $context['type'] ?? null,
    ...
    {
        $cached = RecommendationCache::where('cache_key', $cacheKey)
            ->valid()
            ->first();
    ...
    private function cacheRecommendations(
        string $cacheKey,
        RecommendationBlock $block,
        ?User $user = null,
        ?Product $product = null,
        array $context = [],
        Collection $recommendations = null
    ): void {
    ...
    public function trackUserInteraction(
        ...
        try {
            ...
            Log::error('Failed to track user interaction', [
                'error' => $e->getMessage(),
                'user_id' => $user->id,
                'product_id' => $product->id,
        ...
        foreach ($product->categories as $category) {
            \App\Models\UserPreference::updateOrCreate(
                [
                    ...
                    'preference_key' => $category->id,
                ],
                [
                    'preference_score' => DB::raw("GREATEST(preference_score + {$preferenceScore}, 1.0)"),
                    'last_updated' => now(),
                ]
            );
        }
        ...
        if ($product->brand_id) {
            \App\Models\UserPreference::updateOrCreate(
                [
                    ...
                    'preference_key' => $product->brand_id,
                ],
                [
                    'preference_score' => DB::raw("GREATEST(preference_score + {$preferenceScore}, 1.0)"),
                    'last_updated' => now(),
    ...
    public function getAnalytics(string $blockName, int $days = 30): array
    {
        ...
        try {
            return [
                'block_name' => $blockName,
                'total_requests' => RecommendationCache::where('block_id', $block->id)
                    ->where('created_at', '>=', now()->subDays($days))
                    ->sum('hit_count'),
                'unique_requests' => RecommendationCache::where('block_id', $block->id)
                    ->where('created_at', '>=', now()->subDays($days))
                    ->count(),
                'avg_products_per_request' => RecommendationCache::where('block_id', $block->id)
                    ->where('created_at', '>=', now()->subDays($days))
                    ->avg(DB::raw('JSON_LENGTH(recommendations)')),
                'cache_hit_rate' => $this->calculateCacheHitRate($block->id, $days),
            ];
        } catch (\Exception $e) {
            return [
                'block_name' => $blockName,
    ...
    {
        try {
            $totalRequests = RecommendationCache::where('block_id', $blockId)
                ->where('created_at', '>=', now()->subDays($days))
                ->sum('hit_count');

            $cacheHits = RecommendationCache::where('block_id', $blockId)
                ->where('created_at', '>=', now()->subDays($days))
                ->where('hit_count', '>', 0)
                ->sum('hit_count');
