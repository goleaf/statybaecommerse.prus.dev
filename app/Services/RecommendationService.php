...
final class RecommendationService
{
    ...
    private function getAlgorithmInstance(string $type, array $config = []): BaseRecommendation
    {
        ...
        if (! isset($this->algorithmInstances[$key])) {
            // Allow advanced configurations to provide a fully-qualified
            // algorithm class so bespoke strategies (including test doubles)
            // can be resolved through the container without touching the
            // service internals.
            if (isset($config['algorithm']) && is_string($config['algorithm']) && is_subclass_of($config['algorithm'], BaseRecommendation::class)) {
                $this->algorithmInstances[$key] = app($config['algorithm'], ['config' => $config]);
            } else {
                $this->algorithmInstances[$key] = match ($type) {
                    ...
                    'manual'        => new ManualRecommendation($config),
