    ...
    public function handle(): Order
    {
        ...
        return DB::transaction(function () use ($checkout, $sessionId, $customer) {
            ...
            if ($couponCode) {
                ...
                if ($codeRow) {
                    $now = now();
                    if (($codeRow->expires_at && $now->greaterThan($codeRow->expires_at)) ||
                            ($codeRow->max_uses !== null && $codeRow->usage_count >= $codeRow->max_uses)) {
                        $codeRow = null;  // invalidate
                    }
                }
            }
            ...
            $order = Order::query()->create([
                ...
                'discount_total_amount' => round($discountTotal, 2),
                'tax_total_amount' => round($taxTotal, 2),
                'shipping_total_amount' => round($shippingTotal, 2),
            ...
            foreach ((array) data_get($result, 'applied', []) as $applied) {
                ...
                DB::table('discount_redemptions')->insert([
                    ...
                    'order_id' => $order->id,
                    'user_id' => $customer->id,
                    'amount_saved' => round((float) ($applied['amount'] ?? 0), 2),
                    'currency_code' => current_currency(),
                    'redeemed_at' => now(),
                    'created_at' => now(),
                    'updated_at' => now(),
