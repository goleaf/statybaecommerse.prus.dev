...
final class Index extends Component
{
    ...
    public function render(): View
    {
        ...
        $query = News::query()->select('news.*')->join('sh_news_translations as t', 't.news_id', '=', 'news.id')->where('t.locale', $locale)->where('news.is_visible', true)->whereNotNull('news.published_at')->where('news.published_at', '<=', now());
        $catSlug = request()->query('cat');
        if (is_string($catSlug) && $catSlug !== '') {
            $query->whereExists(function ($q) use ($locale, $catSlug) {
                $q->from('news_category_pivot as ncp')->join('sh_news_category_translations as ct', 'ct.news_category_id', '=', 'ncp.news_category_id')->whereColumn('ncp.news_id', 'news.id')->where('ct.locale', $locale)->where('ct.slug', $catSlug);
            });
        }
