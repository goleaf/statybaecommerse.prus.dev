 ...
 * MobileCategoryMenu
 *
 * Livewire component for MobileCategoryMenu with reactive frontend functionality, real-time updates, and user interaction handling.
 */
    ...
    public function categoryTree()
    {
        ...
        return Cache::remember("mobile_category_tree:{$locale}", now()->addMinutes(30), function () {
            $roots = Category::query()->withProductCounts()->with(['translations' => fn ($q) => $q->where('locale', app()->getLocale()), 'children' => function ($q) {
                ...
                // Limit for mobile
            }])->visible()->roots()->ordered()->limit(8)->get();
