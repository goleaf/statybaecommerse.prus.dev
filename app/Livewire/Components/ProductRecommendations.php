...
final class ProductRecommendations extends Component
{
    ...
    {
        return Product::query()->with(['media', 'brand'])->where('is_visible', true)->withCount(['orderItems' => function ($query) {
            $query->whereHas('order', function ($orderQuery) {
                $orderQuery->where('status', 'completed')->where('created_at', '>=', now()->subMonths(3));
            });
    ...
    private function getPersonalizedRecommendations(): Collection
    {
        ...
        $purchasedCategoryIds = $user->orders()->with('items.product.categories')->where('status', 'completed')->get()->pluck('items')->flatten()->pluck('product.categories')->flatten()->pluck('id')->unique();
    ...
    private function getCrossSellProducts(): Collection
    {
        ...
        $frequentlyBoughtWith = Product::query()->with(['media', 'brand'])->where('is_visible', true)->where('id', '!=', $this->productId)->whereHas('orderItems', function ($query) {
            $query->whereHas('order', function ($orderQuery) {
                $orderQuery->whereIn('status', ['completed', 'delivered'])->whereHas('items', function ($itemQuery) {
    ...
    private function getCustomersAlsoBoughtProducts(): Collection
    {
        ...
        })->whereIn('status', ['completed', 'delivered'])->pluck('id');
        if ($orderIds->isEmpty()) {
    ...
    private function getTrendingProducts(): Collection
    {
        ...
        return Product::query()->with(['media', 'brand'])->where('is_visible', true)->whereHas('orderItems', function ($query) {
            $query->whereHas('order', function ($orderQuery) {
                $orderQuery->whereIn('status', ['completed', 'delivered'])->where('created_at', '>=', now()->subDays(30));
            });
        })->withCount(['orderItems' => function ($query) {
            $query->whereHas('order', function ($orderQuery) {
                $orderQuery->whereIn('status', ['completed', 'delivered'])->where('created_at', '>=', now()->subDays(30));
