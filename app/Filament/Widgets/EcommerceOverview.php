...
final class EcommerceOverview extends BaseWidget
{
    ...
    public function getStatsProperty(): array
    {
        ...
        $revenueThisMonth = (float) (Order::where('status', '!=', 'cancelled')
            ->whereBetween('created_at', [$thisMonthStart, $now])
            ->sum('total') ?? 0);
        $revenueLastMonth = (float) (Order::where('status', '!=', 'cancelled')
        ...
        $ordersThisMonth = (int) Order::whereBetween('created_at', [$thisMonthStart, $now])->count();
        $ordersLastMonth = (int) Order::whereBetween('created_at', [$lastMonthStart, $lastMonthEnd])->count();
        ...
        $activeCampaigns = (int) Campaign::where('is_active', true)->count();
