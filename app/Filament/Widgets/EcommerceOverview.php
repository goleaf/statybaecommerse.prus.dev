...
class EcommerceOverview extends BaseWidget
{
    ...
    {
        $currentMonth = Order::paid()
            ->whereMonth('created_at', now()->month)
            ->sum('total');

        $previousMonth = Order::paid()
    ...
    {
        return number_format(Order::where(function ($q) {
            $q->paid();
        })->count());
    }
    ...
    {
        $count = Order::paid()->distinct('user_id')->count('user_id');
    ...
    {
        $avgValue = Order::paid()->avg('total');
        return '€' . number_format((float) ($avgValue ?? 0), 2);
    }
    ...
    private function getAverageRating(): string
    {
        ...
        return number_format((float) ($avgRating ?? 0), 1) . '/5';
    }
