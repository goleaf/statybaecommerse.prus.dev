...
class AddressStatsWidget extends BaseWidget
{
    protected function getStats(): array
    {
        ...
        $monthlyGrowth = $addressesLastMonth > 0 
            ? round((($addressesThisMonth - $addressesLastMonth) / $addressesLastMonth) * 100, 1)
            : 0;

        $topCountries = Address::select('country_code', DB::raw('count(*) as count'))
        ...
        return [
            Stat::make(__('translations.total_addresses'), $totalAddresses)
                ->description(__('translations.all_addresses_in_system'))
            ...
            Stat::make(__('translations.new_addresses_this_month'), $addressesThisMonth)
                ->description($monthlyGrowth >= 0 
                    ? __('translations.growth_positive', ['percent' => abs($monthlyGrowth)])
                    : __('translations.growth_negative', ['percent' => abs($monthlyGrowth)]))
            ...
            Stat::make(__('translations.top_country'), $topCountries?->country_code ?? 'N/A')
                ->description($topCountries 
                    ? __('translations.addresses_count', ['count' => $topCountries->count])
                    : __('translations.no_data'))
