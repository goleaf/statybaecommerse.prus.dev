        ...
        return $table
            ->query(
                User::query()
                    ...
                    ->withCount(['referrals as completed_referrals' => function (Builder $query) {
                        $query->where('status', 'completed');
                    }])
                    ->withCount(['referrals as pending_referrals' => function (Builder $query) {
                        $query->where('status', 'pending');
                    }])
                    ->withSum('referralRewards', 'amount')
            ...
            ->columns([
                ...
                Tables\Columns\TextColumn::make('completed_referrals')
                    ->label(__('referrals.completed_referrals'))
                    ->sortable()
                    ->badge()
                    ->color('success'),

                Tables\Columns\TextColumn::make('pending_referrals')
                    ->label(__('referrals.pending_referrals'))
                    ->sortable()
                    ->badge()
                    ->color('warning'),

                Tables\Columns\TextColumn::make('referral_rewards_sum_amount')
                    ->label(__('referrals.total_rewards'))
                    ->money('EUR')
                    ->sortable()
                    ->badge()
                    ->color('success'),
