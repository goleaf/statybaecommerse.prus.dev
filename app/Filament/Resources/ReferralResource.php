...
final class ReferralResource extends Resource
{
    ...
    /** @var string|\BackedEnum|null */
    protected static string|UnitEnum|null $navigationGroup = NavigationGroup::Referral;
    ...
    public static function form(Form $form): Form
    {
        return $form
            ->schema([
                Forms\Components\Section::make(__('referrals.referral_information'))
                    ->schema([
                        ...
                        Forms\Components\DateTimePicker::make('expires_at')
                            ...
                            ->nullable()
                            ->helperText(__('referrals.expires_at_help')),
                ...
                Forms\Components\Section::make(__('referrals.translation_information'))
                    ->schema([
                        Forms\Components\TextInput::make('title')
                            ->label(__('referrals.title'))
                            ->maxLength(255)
                            ->translatable(),

                        Forms\Components\Textarea::make('description')
                            ->label(__('referrals.description'))
                            ->rows(3)
                            ->translatable(),

                        Forms\Components\Textarea::make('terms_conditions')
                            ->label(__('referrals.terms_conditions'))
                            ->rows(4)
                            ->translatable(),

                        Forms\Components\Textarea::make('benefits_description')
                            ->label(__('referrals.benefits_description'))
                            ->rows(3)
                            ->translatable(),

                        Forms\Components\Textarea::make('how_it_works')
                            ->label(__('referrals.how_it_works'))
                            ->rows(3)
                            ->translatable(),
                    ])
                    ->columns(1)
                ...
                Forms\Components\Section::make(__('referrals.seo_information'))
                    ->schema([
                        Forms\Components\TextInput::make('seo_title')
                            ->label(__('referrals.seo_title'))
                            ->maxLength(255)
                            ->translatable(),

                        Forms\Components\Textarea::make('seo_description')
                            ->label(__('referrals.seo_description'))
                            ->rows(3)
                            ->translatable(),

                        Forms\Components\TagsInput::make('seo_keywords')
                            ->label(__('referrals.seo_keywords'))
                            ->translatable(),
                    ])
                    ->columns(1)
                    ->collapsible(),

                Forms\Components\Section::make(__('referrals.metadata'))
                    ->schema([
                        Forms\Components\KeyValue::make('metadata')
                            ->label(__('referrals.additional_data'))
                            ->nullable()
                            ->helperText(__('referrals.metadata_help')),
                    ])
                    ->collapsible(),
            ]);
    }
        ...
        return $table
            ->columns([
                ...
                Tables\Columns\TextColumn::make('completed_at')
                    ...
                    ->sortable()
                    ->toggleable()
                    ->placeholder(__('referrals.not_completed')),
            ...
            ->actions([
                ...
                Tables\Actions\Action::make('mark_completed')
                    ...
                    ->action(function (Referral $record): void {
                        ...
                        Notification::make()
                            ->title(__('referrals.referral_updated'))
                            ->success()
                            ->send();
                    })
                ...
                Tables\Actions\BulkActionGroup::make([
                    Tables\Actions\BulkAction::make('mark_completed')
                        ...
                        ->action(function ($records): void {
                            ...
                            Notification::make()
                                ->title(__('referrals.referrals_updated'))
                                ->success()
                                ->send();
                        })
                        ->requiresConfirmation(),
        ...
        return $schema
            ->components([
                ...
                Infolists\Components\Section::make(__('referrals.rewards_section'))
                    ->components([
                        Infolists\Components\RepeatableEntry::make('rewards')
                            ->components([
                                ...
                                Infolists\Components\TextEntry::make('applied_at')
                                    ->label(__('referrals.applied_at'))
                                    ->dateTime()
                                    ->placeholder(__('referrals.not_applied')),
                            ])
                            ->columns(4),
                    ])
                    ->collapsible(),

                Infolists\Components\Section::make(__('referrals.metadata'))
                    ->components([
    ...
    {
        return [
            Widgets\ReferralStatsWidget::class,
            Widgets\TopReferrersWidget::class,
            Widgets\RecentReferralsWidget::class,
            Widgets\ReferralTrendsWidget::class,
        ];
    }
    ...
    {
        return [
            __('referrals.referrer') => $record->referrer->name,
            __('referrals.referred_user') => $record->referred->name,
            __('referrals.status') => $record->status,
            __('referrals.created_at') => $record->created_at->format('Y-m-d H:i'),
        ];
    }
