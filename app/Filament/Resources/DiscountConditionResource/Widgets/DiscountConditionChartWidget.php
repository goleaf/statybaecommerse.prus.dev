 ...
 * 
 * Filament v4 resource for DiscountConditionChartWidget management in the admin panel with comprehensive CRUD operations, filters, and actions.
 * 
...
final class DiscountConditionChartWidget extends ChartWidget
{
    ...
    {
        $data = DiscountCondition::select('type', DB::raw('count(*) as count'))->groupBy('type')->get()->pluck('count', 'type')->toArray();
        $labels = [];
        $values = [];
        ...
        return ['datasets' => [['label' => __('discount_conditions.charts.conditions_by_type'), 'data' => $values, 'backgroundColor' => [
            '#3B82F6',
            // blue
            '#10B981',
            // emerald
            ...
            // lime
            '#F97316',
        ], 'borderColor' => '#ffffff', 'borderWidth' => 2]], 'labels' => $labels];
    }
    /**
