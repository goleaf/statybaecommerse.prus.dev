...
class PriceListResource extends Resource
{
                ...
                Section::make(__('admin.price_lists.basic_information'))
                    ->schema([
                        Grid::make(2)
                            ->schema([
                                TextInput::make('name')
                                    ->label(__('admin.price_lists.fields.name'))
                                    ->required()
                                    ->maxLength(255)
                                    ->live()
                                ...
                                Select::make('currency_id')
                                    ->label(__('admin.price_lists.fields.currency'))
                                ...
                                Select::make('zone_id')
                                    ->label(__('admin.price_lists.fields.zone'))
                        ...
                        Grid::make(3)
                            ->schema([
                                Toggle::make('is_enabled')
                                    ->label(__('admin.price_lists.fields.is_enabled'))
                                    ->default(true),
                                Toggle::make('is_default')
                                    ->label(__('admin.price_lists.fields.is_default'))
                                    ->default(false),
                                Toggle::make('auto_apply')
                                    ->label(__('admin.price_lists.fields.auto_apply'))
                                    ->default(false),
                                TextInput::make('priority')
                                    ->label(__('admin.price_lists.fields.priority'))
                                    ->numeric()
                                    ->default(100)
                                    ->minValue(1)
                                    ->maxValue(999),
                                TextInput::make('min_order_amount')
                                    ->label(__('admin.price_lists.fields.min_order_amount'))
                                    ->numeric()
                                    ->prefix('€')
                                    ->step(0.01),
                                TextInput::make('max_order_amount')
                                    ->label(__('admin.price_lists.fields.max_order_amount'))
                                    ->numeric()
                                    ->prefix('€')
                                    ->step(0.01),
                            ]),
                        ...
                        Grid::make(2)
                            ->schema([
                                DateTimePicker::make('starts_at')
                                    ->label(__('admin.price_lists.fields.starts_at'))
                                    ->displayFormat('d/m/Y H:i'),
                                DateTimePicker::make('ends_at')
                                    ->label(__('admin.price_lists.fields.ends_at'))
                                    ->displayFormat('d/m/Y H:i')
                                    ->after('starts_at'),
                            ]),
                    ]),
                Section::make(__('admin.price_lists.associations'))
                    ->schema([
                        Select::make('customerGroups')
                            ->label(__('admin.price_lists.fields.customer_groups'))
                            ->relationship('customerGroups', 'name')
                            ->multiple()
                            ->searchable()
                            ->preload(),
                        Select::make('partners')
                            ->label(__('admin.price_lists.fields.partners'))
                ...
                Section::make(__('admin.price_lists.metadata'))
                    ->schema([
                        KeyValue::make('metadata')
                            ->label(__('admin.price_lists.fields.metadata'))
                            ->keyLabel(__('admin.price_lists.fields.metadata_key'))
                            ->valueLabel(__('admin.price_lists.fields.metadata_value')),
                    ])
                    ->collapsible(),
            ]);
    }
        ...
        return $table
            ->columns([
                ...
                TextColumn::make('items_count')
                    ->label(__('admin.price_lists.fields.items_count'))
                    ->counts('items')
                    ->numeric()
                    ->alignEnd(),
                TextColumn::make('customer_groups_count')
                    ->label(__('admin.price_lists.fields.customer_groups_count'))
                    ->counts('customerGroups')
                    ->numeric()
                    ->alignEnd(),
                TextColumn::make('partners_count')
                    ->label(__('admin.price_lists.fields.partners_count'))
                    ->counts('partners')
                    ->numeric()
                    ->alignEnd(),
    ...
    {
        return [
            Widgets\PriceListStatsWidget::class,
            Widgets\PriceListActivityWidget::class,
        ];
    }
