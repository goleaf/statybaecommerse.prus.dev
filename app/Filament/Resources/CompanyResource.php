                    ...
                    Grid::make(2)
                        ->schema([
                            TextInput::make('website')
                                ...
                                ->maxLength(255),
                            Select::make('country')
                                ->label(__('companies.country'))
                                ->options(function (): array {
                                    return collect(\App\Models\Country::query()->orderBy('name')->pluck('name', 'id')->all())
                                        ->mapWithKeys(fn($name, $id) => [$id => is_array($name) ? ($name[app()->getLocale()] ?? ($name['lt'] ?? $name['en'] ?? reset($name))) : $name])
                                        ->toArray();
                                })
                                ->searchable()
                                ->preload(),
