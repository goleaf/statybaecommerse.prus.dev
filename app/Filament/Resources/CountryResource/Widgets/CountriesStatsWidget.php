...
class CountriesStatsWidget extends BaseWidget
{
    protected function getStats(): array
    {
        ...
        return [
            Stat::make(__('admin.countries.statistics.total_countries'), $totalCountries)
                ->description(__('admin.countries.statistics.total_countries_description'))
                ->descriptionIcon('heroicon-m-globe-alt')
                ->color('primary'),

            Stat::make(__('admin.countries.statistics.active_countries'), $activeCountries)
                ->description(__('admin.countries.statistics.active_countries_description'))
                ->descriptionIcon('heroicon-m-check-circle')
                ->color('success'),

            Stat::make(__('admin.countries.statistics.eu_members'), $euMembers)
                ->description(__('admin.countries.statistics.eu_members_description'))
                ->descriptionIcon('heroicon-m-building-office-2')
                ->color('info'),

            Stat::make(__('admin.countries.statistics.countries_with_vat'), $countriesWithVat)
