...
class CategoryResource extends Resource
{
        ...
        return $schema
            ->schema([
                Tabs::make(__('categories.tabs.translations'))
                    ->tabs([
                        Tab::make(__('categories.tabs.lithuanian'))
                            ->icon('heroicon-o-language')
                            ->schema([
                                Section::make(__('categories.sections.basic_information'))
                                    ->schema([
                                        TextInput::make('translations.name.lt')
                ...
                Section::make(__('categories.sections.hierarchy'))
                    ->schema([
                        Repeater::make('children')
                            ->label(__('categories.fields.children'))
        ...
        return $table
            ...
            ->filters([
                TernaryFilter::make('is_enabled')
                    ->label(__('categories.filters.is_enabled'))
                ...
                SelectFilter::make('has_children')
                    ...
                    ->options([
                        'with_children' => __('categories.filters.with_children'),
                ...
                SelectFilter::make('has_products')
                    ...
                    ->options([
                        'with_products' => __('categories.filters.with_products'),
                        'without_products' => __('categories.filters.without_products'),
                    ])
                    ->query(function (Builder $query, array $data): Builder {
                        return match ($data['value'] ?? null) {
                ...
                SelectFilter::make('products_count_range')
                    ...
                    ->options([
                        '0' => __('categories.filters.no_products'),
                        '1-10' => __('categories.filters.1_to_10_products'),
                        '11-50' => __('categories.filters.11_to_50_products'),
                        '51-100' => __('categories.filters.51_to_100_products'),
                        '100+' => __('categories.filters.100_plus_products'),
                    ])
                    ->query(function (Builder $query, array $data): Builder {
                        return match ($data['value'] ?? null) {
                ...
                Filter::make('created_at')
                    ->form([
                        Forms\Components\DatePicker::make('created_from')
                            ->label(__('categories.filters.created_from')),
                        Forms\Components\DatePicker::make('created_until')
                            ->label(__('categories.filters.created_until')),
                    ])
                    ->query(function (Builder $query, array $data): Builder {
                        return $query
                            ->when(
            ...
            ->bulkActions([
                BulkActionGroup::make([
                    Action::make('enable_selected')
                        ->label(__('categories.bulk_actions.enable_selected'))
                        ->icon('heroicon-o-check-circle')
                        ->color('success')
                        ->action(function (Collection $records) {
                            $records->each->update(['is_enabled' => true]);
                            Notification::make()
                                ->title(__('categories.messages.status_changed'))
                                ->success()
                                ->send();
                        }),
                    Action::make('disable_selected')
                        ->label(__('categories.bulk_actions.disable_selected'))
                    ...
                    Action::make('feature_selected')
                        ->label(__('categories.bulk_actions.feature_selected'))
                        ->icon('heroicon-o-star')
                        ->color('warning')
                        ->action(function (Collection $records) {
                            $records->each->update(['is_featured' => true]);
                            Notification::make()
                                ->title(__('categories.messages.featured_toggled'))
                                ->success()
                                ->send();
                        }),
            ...
            ->poll('30s')
            ->emptyStateHeading(__('categories.messages.no_categories_found'))
            ->emptyStateDescription(__('categories.help.create_first_category'))
    ...
    {
        return [
            Widgets\CategoryStatsWidget::class,
            Widgets\CategoryTreeWidget::class,
    ...
    {
        return [
            __('categories.fields.parent') => $record->parent?->trans('name', app()->getLocale()) ?: $record->parent?->trans('name', 'lt'),
            __('categories.fields.products_count') => $record->products_count,
            __('categories.fields.children_count') => $record->children_count,
        ];
    }
