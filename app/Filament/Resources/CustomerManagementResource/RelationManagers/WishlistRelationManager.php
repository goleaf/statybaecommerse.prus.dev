    ...
    public function form(Form $form): Form
    {
        return $schema->schema([
                ...
                Forms\Components\Select::make('variant_id')
                    ->label(__('admin.customers.fields.variant'))
                ...
                Forms\Components\DateTimePicker::make('added_at')
                    ->label(__('admin.customers.fields.added_at'))
                    ->required(),
            ]);
    }
        ...
        return $table
            ...
            ->columns([
                Tables\Columns\ImageColumn::make('product.image')
                    ->label(__('admin.customers.fields.image'))
                    ->circular()
                ...
                Tables\Columns\TextColumn::make('variant.name')
                    ->label(__('admin.customers.fields.variant'))
                    ->searchable()
                    ->placeholder(__('admin.no_variant')),

                Tables\Columns\TextColumn::make('product.price')
                    ->label(__('admin.customers.fields.price'))
                    ->money('EUR')
                    ->sortable(),

                Tables\Columns\TextColumn::make('product.stock_quantity')
                    ->label(__('admin.customers.fields.stock'))
                    ->numeric()
                    ->sortable()
                ...
                Tables\Columns\TextColumn::make('notes')
                    ...
                    ->searchable()
                    ->limit(50)
                    ->placeholder(__('admin.customers.no_notes')),
            ...
            ->filters([
                Tables\Filters\TernaryFilter::make('has_variant')
                    ->label(__('admin.customers.fields.has_variant'))
                    ->nullable()
                    ->trueLabel(__('admin.with_variant'))
                    ->falseLabel(__('admin.without_variant'))
                    ->queries(
                ...
                Tables\Filters\Filter::make('added_at')
                    ->label(__('admin.customers.fields.added_at'))
                    ->form([
                        Forms\Components\DatePicker::make('added_from')
                            ->label(__('admin.customers.filters.added_from')),
                        Forms\Components\DatePicker::make('added_until')
                            ->label(__('admin.customers.filters.added_until')),
                    ])
                    ->query(function (Builder $query, array $data): Builder {
                        return $query
                            ->when(
                                $data['added_from'],
                                fn (Builder $query, $date): Builder => $query->whereDate('added_at', '>=', $date),
                            )
                            ->when(
                                $data['added_until'],
                                fn (Builder $query, $date): Builder => $query->whereDate('added_at', '<=', $date),
            ...
            ->headerActions([
                Tables\Actions\CreateAction::make()
                    ->label(__('admin.customers.add_to_wishlist')),
            ])
            ->actions([
