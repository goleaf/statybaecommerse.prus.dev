...
final class ActivityLogResource extends Resource
{
                ...
                Forms\Components\Section::make(__('admin.activity_logs.sections.activity_information'))
                    ->schema([
                        Forms\Components\TextInput::make('log_name')
                            ->label(__('admin.activity_logs.fields.log_name'))
                            ->maxLength(255)
                            ->required(),
                        Forms\Components\TextInput::make('description')
                            ->label(__('admin.activity_logs.fields.description'))
                            ->maxLength(255)
                            ->required()
                            ->columnSpanFull(),
                        ...
                        Forms\Components\Select::make('causer_type')
                            ->label(__('admin.activity_logs.fields.causer_type'))
                            ->options([
                                'App\Models\User' => __('admin.activity_logs.subject_types.App\Models\User'),
                            ])
                            ->searchable(),
                        Forms\Components\TextInput::make('causer_id')
                            ->label(__('admin.activity_logs.fields.causer_id'))
                            ->numeric(),
                    ])
                    ->columns(2),
                Forms\Components\Section::make(__('admin.activity_logs.sections.additional_data'))
                    ->schema([
                        Forms\Components\KeyValue::make('properties')
                            ->label(__('admin.activity_logs.fields.properties'))
                            ->keyLabel(__('admin.activity_logs.fields.property_key'))
                            ->valueLabel(__('admin.activity_logs.fields.property_value'))
                            ->columnSpanFull(),
                        Forms\Components\TextInput::make('event')
                            ->label(__('admin.activity_logs.fields.event'))
                            ->maxLength(255),
                        Forms\Components\TextInput::make('batch_uuid')
                            ->label(__('admin.activity_logs.fields.batch_uuid'))
                            ->maxLength(255),
                    ])
                    ->columns(2),
            ]);
        ...
        return $table
            ->columns([
                ...
                Tables\Columns\TextColumn::make('causer.name')
                    ->label(__('admin.activity_logs.fields.causer'))
                    ->searchable()
                    ->sortable()
                    ->badge()
                    ->color('success'),
                Tables\Columns\TextColumn::make('created_at')
                    ->label(__('admin.activity_logs.fields.created_at'))
                    ->dateTime()
                    ->sortable()
                    ->toggleable(isToggledHiddenByDefault: true),
                Tables\Columns\TextColumn::make('updated_at')
                    ->label(__('admin.activity_logs.fields.updated_at'))
                    ->dateTime()
                    ->sortable()
                    ->toggleable(isToggledHiddenByDefault: true),
            ])
                ...
                Tables\Filters\Filter::make('created_at')
                    ->form([
                        Forms\Components\DatePicker::make('created_from')
                            ->label(__('admin.activity_logs.filters.created_from')),
                        Forms\Components\DatePicker::make('created_until')
                            ->label(__('admin.activity_logs.filters.created_until')),
                    ])
    ...
    public static function getRelations(): array
    {
        return [
            ...
            RelationManagers\CauserRelationManager::class,
        ];
    }
    ...
    {
        return [
            Widgets\ActivityLogStatsWidget::class,
            Widgets\ActivityLogChartWidget::class,
            Widgets\RecentActivityLogsWidget::class,
        ];
    }
