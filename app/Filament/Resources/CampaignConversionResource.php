    ...
    public static function form(Form $form): Form
    {
        return $form
            ->schema([
                        ...
                        Grid::make(3)
                            ->schema([
                                Select::make('campaign_id')
                                    ...
                                    ->createOptionForm([
                                        TextInput::make('name')
                                            ->label(__('campaigns.form.name'))
                                            ->required()
                                            ->maxLength(255),
                                        TextInput::make('slug')
                                            ->label(__('campaigns.form.slug'))
                                            ->required()
                                            ->maxLength(255),
                                    ]),
                ...
                Section::make(__('campaign_conversions.form.analytics'))
                    ->schema([
                        Grid::make(3)
                            ->schema([
                                TextInput::make('conversion_duration')
                                    ->label(__('campaign_conversions.form.conversion_duration'))
                                    ->numeric()
                                    ->suffix(__('campaign_conversions.form.seconds')),
                                TextInput::make('page_views')
                                    ->label(__('campaign_conversions.form.page_views'))
                                    ->numeric(),
                                TextInput::make('time_on_site')
                                    ->label(__('campaign_conversions.form.time_on_site'))
                                    ->numeric()
                                    ->suffix(__('campaign_conversions.form.seconds')),
                            ]),
                        Grid::make(2)
                            ->schema([
                                TextInput::make('bounce_rate')
                                    ->label(__('campaign_conversions.form.bounce_rate'))
                                    ->numeric()
                                    ->step(0.01)
                                    ->suffix('%'),
                        ...
                        Grid::make(2)
                            ->schema([
                                TextInput::make('exit_page')
                                    ->label(__('campaign_conversions.form.exit_page'))
                                    ->maxLength(500),
                                TextInput::make('landing_page')
                                    ->label(__('campaign_conversions.form.landing_page'))
                                    ->maxLength(500),
                            ]),
                    ]),
                Section::make(__('campaign_conversions.form.attribution'))
                    ->schema([
                        Select::make('attribution_model')
                            ->label(__('campaign_conversions.form.attribution_model'))
                            ->options([
                                'last_click' => __('campaign_conversions.attribution_models.last_click'),
                                'first_click' => __('campaign_conversions.attribution_models.first_click'),
                                'linear' => __('campaign_conversions.attribution_models.linear'),
                                'time_decay' => __('campaign_conversions.attribution_models.time_decay'),
                                'position_based' => __('campaign_conversions.attribution_models.position_based'),
                                'data_driven' => __('campaign_conversions.attribution_models.data_driven'),
                            ])
                            ->default('last_click'),
                        Grid::make(3)
                            ->schema([
                                TextInput::make('last_click_attribution')
                                    ->label(__('campaign_conversions.form.last_click_attribution'))
                                    ->numeric()
                                    ->prefix('€')
                                    ->step(0.01),
                                TextInput::make('first_click_attribution')
                                    ->label(__('campaign_conversions.form.first_click_attribution'))
                                    ->numeric()
                                    ->prefix('€')
                                    ->step(0.01),
                                TextInput::make('linear_attribution')
                                    ->label(__('campaign_conversions.form.linear_attribution'))
                                    ->numeric()
                                    ->prefix('€')
                                    ->step(0.01),
                            ]),
                        Grid::make(2)
                            ->schema([
                                TextInput::make('conversion_window')
                                    ->label(__('campaign_conversions.form.conversion_window'))
                                    ->numeric()
                                    ->suffix(__('campaign_conversions.form.days'))
                                    ->default(30),
                                TextInput::make('lookback_window')
                                    ->label(__('campaign_conversions.form.lookback_window'))
                                    ->numeric()
                                    ->suffix(__('campaign_conversions.form.days'))
                                    ->default(90),
                            ]),
                    ]),
                Section::make(__('campaign_conversions.form.performance_metrics'))
                    ->schema([
                        Grid::make(3)
                            ->schema([
                                TextInput::make('cost_per_conversion')
                                    ->label(__('campaign_conversions.form.cost_per_conversion'))
                                    ->numeric()
                                    ->prefix('€')
                                    ->step(0.01),
                                TextInput::make('roi')
                                    ->label(__('campaign_conversions.form.roi'))
                                    ->numeric()
                                    ->step(0.0001)
                                    ->suffix('%'),
                                TextInput::make('roas')
                                    ->label(__('campaign_conversions.form.roas'))
                                    ->numeric()
                                    ->step(0.0001)
                                    ->suffix('%'),
                            ]),
                        Grid::make(3)
                            ->schema([
                                TextInput::make('lifetime_value')
                                    ->label(__('campaign_conversions.form.lifetime_value'))
                                    ->numeric()
                                    ->prefix('€')
                                    ->step(0.01),
                                TextInput::make('customer_acquisition_cost')
                                    ->label(__('campaign_conversions.form.customer_acquisition_cost'))
                                    ->numeric()
                                    ->prefix('€')
                                    ->step(0.01),
                                TextInput::make('payback_period')
                                    ->label(__('campaign_conversions.form.payback_period'))
                                    ->numeric()
                                    ->suffix(__('campaign_conversions.form.days')),
                            ]),
                    ]),
                Section::make(__('campaign_conversions.form.additional_information'))
                    ->schema([
                        Textarea::make('notes')
                            ...
                            ->columnSpanFull(),
                        TagsInput::make('tags')
                            ->label(__('campaign_conversions.form.tags'))
                            ->columnSpanFull(),
                        KeyValue::make('custom_attributes')
                            ->label(__('campaign_conversions.form.custom_attributes'))
                            ->columnSpanFull(),
                        KeyValue::make('conversion_data')
                            ->label(__('campaign_conversions.form.conversion_data'))
                            ->columnSpanFull(),
                    ]),
            ]);
    }
        ...
        return $table
            ...
            ->filters([
                ...
                TernaryFilter::make('is_tablet')
                    ->label(__('campaign_conversions.filters.tablet')),
                TernaryFilter::make('is_desktop')
                    ->label(__('campaign_conversions.filters.desktop')),
            ])
            ->actions([
                ...
                InfolistSection::make(__('campaign_conversions.infolist.additional_information'))
                    ->schema([
                        TextEntry::make('notes')
                            ...
                            ->columnSpanFull(),
                        KeyValueEntry::make('custom_attributes')
                            ->label(__('campaign_conversions.infolist.custom_attributes'))
                            ->columnSpanFull(),
                        KeyValueEntry::make('conversion_data')
                            ->label(__('campaign_conversions.infolist.conversion_data'))
                            ->columnSpanFull(),
                    ]),
            ]);
    }
