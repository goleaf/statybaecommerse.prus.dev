@php
    $code = $code ?? 'Error';
    $title = $title ?? __('frontend.errors.generic_title');
    $description = $description ?? __('frontend.errors.generic_description');
...
@extends('components.layouts.base', ['suppressLivewire' => true])

@section('title', $title . ' - ' . config('app.name'))

@section('meta')
    <x-seo-meta :title="$title . ' - ' . config('app.name')" :description="$description" :noindex="true" />
@endsection

@section('content')
<div
    class="min-h-screen bg-gradient-to-b from-white via-gray-50 to-gray-100 flex flex-col justify-center py-16 px-6 lg:px-8">
    <div class="sm:mx-auto sm:w-full sm:max-w-2xl">
        <div class="bg-white shadow-soft rounded-3xl p-10 text-center border border-gray-100">
            <div class="mb-6 flex justify-center">
                <span
                    class="inline-flex items-center justify-center rounded-full bg-blue-100 text-blue-600 h-16 w-16 font-semibold text-2xl">{{ $code }}</span>
            </div>
            ...
            <div class="flex flex-col sm:flex-row gap-4 justify-center">
                ...
                @elseif($primaryType === 'back')
                    <button onclick="history.back()" class="btn-gradient px-8 py-3 rounded-xl font-semibold text-center">
                        ...
                        @foreach($topCategories as $category)
                            <a href="{{ $category['url'] }}"
                                ...
                                <div class="flex items-start gap-4">
                                    <div
                                        class="flex h-12 w-12 items-center justify-center rounded-xl bg-blue-50 text-blue-600 font-semibold">
                                        {{ $category['label'][0] ?? '•' }}
                                    </div>
                                    <div class="flex-1">
                                        <div class="flex items-center justify-between gap-2">
                                            ...
                                            @if(!empty($category['product_count']))
                                                <span
                                                    class="text-xs font-medium text-blue-600 bg-blue-50 px-2.5 py-1 rounded-full">{{ trans_choice('messages.frontend_collections', (int) $category['product_count'], ['count' => (int) $category['product_count']]) }}</span>
                                            @endif
                                        </div>
            ...
            <div class="mt-12">
                <div
                    class="bg-blue-50 border border-blue-100 rounded-3xl p-8 text-left sm:flex sm:items-center sm:justify-between sm:gap-8">
                    <div class="max-w-xl">
                        <h3 class="text-2xl font-semibold text-gray-900 mb-3">
                            {{ $contactCta['title'] ?? __('frontend.errors.contact_cta.title') }}</h3>
                        <p class="text-sm text-gray-600">
                            {{ $contactCta['description'] ?? __('frontend.errors.contact_cta.description') }}</p>
                    </div>
