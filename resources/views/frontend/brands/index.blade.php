        ...
        <div class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
            <header class="rounded-3xl bg-gradient-to-r from-purple-600 via-fuchsia-500 to-rose-500 p-10 text-white shadow-xl">
                <div class="flex flex-col gap-6 lg:flex-row lg:items-center lg:justify-between">
                    <div class="max-w-2xl space-y-3">
                        <span class="inline-flex items-center gap-2 rounded-full bg-white/10 px-3 py-1 text-xs font-semibold uppercase tracking-wide">{{ __('ui.browse_by_brand') }}</span>
                        <h1 class="text-3xl font-semibold sm:text-4xl">{{ __('ui.featured_catalogue_brands') }}</h1>
                        <p class="text-sm text-white/80 sm:text-base">{{ __('ui.verified_suppliers_with_active_inventory_and_real_time_product_availability_across_multiple_categories') }}</p>
                    </div>
                    <div class="grid gap-3 text-sm text-white/80">
                        <div class="flex items-center justify-between rounded-3xl bg-white/10 px-5 py-3">
                            <span>{{ __('ui.brands_highlighted_today') }}</span>
                            <span class="text-lg font-semibold text-white">{{ number_format($highlightedBrands->count()) }}</span>
                        </div>
                        <div class="flex items-center justify-between rounded-3xl bg-white/10 px-5 py-3">
                            <span>{{ __('ui.featured_products_ready_to_ship') }}</span>
            ...
            <section class="mt-10 grid gap-8 lg:grid-cols-[1fr_320px]">
                ...
                <aside class="space-y-6">
                    <div class="rounded-3xl border border-gray-200 bg-white p-6 shadow-sm">
                        <h2 class="text-lg font-semibold text-gray-900">{{ __('ui.trending_brands') }}</h2>
