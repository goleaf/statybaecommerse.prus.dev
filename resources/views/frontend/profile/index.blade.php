<x-layouts.base title="{{ __('messages.frontend) }}">
    <div class="max-w-4xl mx-auto px-4 py-10 space-y-8">
        <header class="flex items-center justify-between">
            ...
            <a href="{{ route('frontend.profile.edit') }}" class="inline-flex items-center px-4 py-2 bg-primary-600 text-white rounded-lg hover:bg-primary-700">{{ __('messages.frontend) }}</a>
        </header>
        ...
        <section class="bg-white dark:bg-gray-900 border border-gray-200 dark:border-white/10 rounded-xl shadow-sm p-6">
            ...
            <dl class="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm text-gray-600 dark:text-gray-300">
                <div>
                    <dt class="font-medium text-gray-900 dark:text-gray-100">{{ __('messages.frontend) }}</dt>
                    <dd>{{ $user->name }}</dd>
                </div>
                <div>
        ...
        <section class="bg-white dark:bg-gray-900 border border-gray-200 dark:border-white/10 rounded-xl shadow-sm p-6">
            <div class="flex items-center justify-between mb-4">
                <h2 class="text-xl font-semibold">{{ __('messages.frontend) }}</h2>
                    ...
                    @foreach ($user->addresses as $address)
                        <div class="p-4 border border-gray-200 dark:border-white/10 rounded-lg">
                            ...
                            @if ($address->is_default)
                                <span class="inline-flex items-center mt-2 px-2 py-1 text-xs font-semibold rounded-full bg-primary-100 text-primary-700">{{ __('messages.frontend) }}</span>
                            @endif
                        </div>
                    @endforeach
                </div>
