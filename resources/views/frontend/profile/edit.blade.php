<x-layouts.base title="{{ __('frontend.profile.edit.title') }}">
    <div class="max-w-3xl mx-auto px-4 py-10 space-y-6">
        <h1 class="text-3xl font-semibold text-gray-900 dark:text-gray-100">{{ __('frontend.profile.edit.heading') }}</h1>
        <form method="POST" action="{{ route('frontend.profile.update') }}" class="bg-white dark:bg-gray-900 border border-gray-200 dark:border-white/10 rounded-xl shadow-sm p-6 space-y-4">
            ...
            @method('PUT')
            <div>
                <label class="block text-sm font-medium text-gray-700 dark:text-gray-300" for="name">{{ __('frontend.profile.fields.name') }}</label>
            ...
            <div>
                <label class="block text-sm font-medium text-gray-700 dark:text-gray-300" for="password">{{ __('frontend.profile.fields.password') }}</label>
            ...
            <div class="flex justify-end gap-4">
                <a href="{{ route('frontend.profile.index') }}" class="px-4 py-2 border border-gray-300 rounded-lg text-sm hover:bg-gray-50">{{ __('frontend.profile.edit.actions.cancel') }}</a>
                <button type="submit" class="inline-flex items-center px-4 py-2 bg-primary-600 text-white rounded-lg hover:bg-primary-700">{{ __('frontend.profile.edit.actions.save') }}</button>
            </div>
        </form>
    </div>
</x-layouts.base>
