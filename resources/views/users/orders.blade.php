    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                ...
                <li>
                    <div class="flex items-center">
                        ...
                        <a href="{{ route('users.dashboard') }}"
                           class="ml-1 text-sm font-medium text-gray-700 hover:text-blue-600 md:ml-2">{{ __('users.dashboard') }}</a>
                    </div>
                </li>
        ...
        <div class="bg-white rounded-lg shadow-sm border border-gray-200 p-6 mb-8">
            <form method="GET" action="{{ route('users.orders') }}" class="grid grid-cols-1 md:grid-cols-4 gap-4">
                ...
                <div>
                    <label for="status"
                           class="block text-sm font-medium text-gray-700 mb-2">{{ __('users.filter_by_status') }}</label>
                    <select
                            id="status"
                            name="status"
                ...
                <div>
                    <label for="date_from"
                           class="block text-sm font-medium text-gray-700 mb-2">{{ __('users.date_from') }}</label>
                    <input
                           type="date"
                           id="date_from"
                           name="date_from"
                ...
                <div>
                    <label for="date_to"
                           class="block text-sm font-medium text-gray-700 mb-2">{{ __('users.date_to') }}</label>
                    <input
                           type="date"
                           id="date_to"
                           name="date_to"
                            ...
                            <div
                                ...
                                <div class="flex space-x-4">
                                    ...
                                    @if (in_array($order->status, ['pending', 'processing']) && $order->can_be_cancelled)
                                        <form method="POST" action="{{ route('users.orders.cancel', $order) }}"
                                              class="inline">
                                            @csrf
                                            @method('PUT')
                                            <button
                                ...
                                <div class="text-sm text-gray-500">
                                    ...
                                    @if ($order->estimated_delivery_date)
                                        <p>{{ __('users.estimated_delivery') }}:
                                            {{ $order->estimated_delivery_date->format('Y-m-d') }}</p>
                                    @endif
                                </div>
                            </div>
                        </div>
