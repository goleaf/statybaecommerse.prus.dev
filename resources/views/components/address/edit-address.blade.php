    ...
    @if ($address->type === 'billing')
        <div class="absolute top-2 right-2">
            <span
                ...
                {{ __('frontend.account.addresses.types.billing') }}
            </span>
        </div>
    @endif
    ...
    <div class="flex flex-col justify-between flex-1">
            ...
            <div class="space-y-2">
                @if ($address->type === 'shipping' && $address->is_default)
                    <div class="flex items-center gap-2 text-sm">
                        ...
                        <span class="text-gray-600">
                            {{ __('frontend.account.addresses.defaults.shipping') }}
                        </span>
                    </div>
                @endif
                @if ($address->type === 'billing' && $address->is_default)
                    <div class="flex items-center gap-2 text-sm">
                        <x-heroicon-o-check class="text-gray-400 size-5" stroke-width="1.5" aria-hidden="true" />
                        <span class="text-gray-600">
                            {{ __('frontend.account.addresses.defaults.billing') }}
                        </span>
                    </div>
                @endif
            </div>
        ...
        <div class="flex items-center gap-4">
            ...
            >
                ...
                <span class="sr-only">{{ __('frontend.buttons.delete') }}</span>
            </button>
            <button
                type="button"
            ...
            >
                ...
                <span class="sr-only">{{ __('frontend.buttons.edit') }}</span>
            </button>
        </div>
    </div>
</div>
