    ...
    <x-container class="relative z-10">
        <div class="grid grid-cols-1 gap-12 py-20 lg:grid-cols-3 lg:py-28">
            <div class="lg:max-w-sm">
                <h3 class="text-sm font-medium text-white mb-6">
                    <x-brand
                             class="w-auto h-12 lg:h-14 text-white"
                             aria-hidden="true" />
                </h3>
            ...
            <div class="space-y-12 lg:grid lg:grid-cols-2 lg:gap-12 lg:space-y-0 lg:col-span-2">
                <div class="grid grid-cols-2 gap-8 lg:gap-12">
                    <div>
                        <h3 class="font-heading text-sm font-medium uppercase tracking-wider text-gray-300 mb-8">
                            {{ __('footer_about') }}
                        </h3>
                        <ul role="list" class="space-y-4">
                            <li>
                                <x-footer-link :spa="false" href="https://filamentphp.com/docs" class="text-gray-400 hover:text-white transition-colors duration-200">
                                    {{ __('footer_documentation') }}
                                </x-footer-link>
                            </li>
                            <li>
                                <x-footer-link :spa="false" href="https://github.com/filamentphp/filament" class="text-gray-400 hover:text-white transition-colors duration-200">
                                    {{ __('footer_github') }}
                                </x-footer-link>
                            </li>
                            <li>
                                <x-footer-link href="#" class="text-gray-400 hover:text-white transition-colors duration-200">{{ __('footer_about') }}</x-footer-link>
                            </li>
                            <li>
                                <x-footer-link href="#" class="text-gray-400 hover:text-white transition-colors duration-200">{{ __('footer_secure_payment') }}</x-footer-link>
                            </li>
                            <li>
                                <x-footer-link href="/" class="text-gray-400 hover:text-white transition-colors duration-200">{{ __('footer_contact') }}</x-footer-link>
                            </li>
                        </ul>
                    </div>
                    <div>
                        ...
                        <ul role="list" class="space-y-4">
                            ...
                            @if ((bool) ($features['category'] ?? false))
                                <li>
                                    <x-footer-link
                                                   href="{{ route('categories.index', ['locale' => app()->getLocale()]) }}" class="text-gray-400 hover:text-white transition-colors duration-200">{{ __('nav_categories') }}</x-footer-link>
                                </li>
                            @endif
                            @if ((bool) ($features['collection'] ?? false) && \Illuminate\Support\Facades\Route::has('collections.index'))
                                <li>
                                    <x-footer-link
                                                   href="{{ route('collections.index', ['locale' => app()->getLocale()]) }}" class="text-gray-400 hover:text-white transition-colors duration-200">{{ __('nav_collections') }}</x-footer-link>
                                </li>
                            @endif
                            @if ((bool) ($features['brand'] ?? false))
                                <li>
                                    <x-footer-link
                                                   href="{{ route('brands.index', ['locale' => app()->getLocale()]) }}" class="text-gray-400 hover:text-white transition-colors duration-200">{{ __('nav_brands') }}</x-footer-link>
                                </li>
                            @endif
                            <li>
                                <x-footer-link href="#" class="text-gray-400 hover:text-white transition-colors duration-200">{{ __('legal_shipping') }}</x-footer-link>
                            </li>
                            <li>
                                <x-footer-link href="#" class="text-gray-400 hover:text-white transition-colors duration-200">{{ __('footer_returns_refunds') }}</x-footer-link>
                            </li>
                        </ul>
                    </div>
                </div>
                <div>
                    ...
                    <div class="space-y-8">
                            ...
                            <form class="space-y-4" target="_blank" novalidate>
                                <div class="flex flex-col sm:flex-row gap-3">
                                    <input
                                           ...
                                           class="flex-1 rounded-xl border-gray-600 bg-gray-800/50 px-4 py-3 text-sm text-white placeholder:text-gray-400 focus:border-blue-500 focus:ring-2 focus:ring-blue-500/20 focus:bg-gray-800 transition-all duration-200"
                                           required />
                                    <button
                                            type="submit"
                                            class="inline-flex items-center gap-2 rounded-xl bg-gradient-to-r from-blue-500 to-blue-600 px-6 py-3 text-sm font-medium text-white hover:from-blue-600 hover:to-blue-700 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 focus:ring-offset-gray-800 transition-all duration-200 transform hover:scale-105">
                        ...
                        <div class="flex items-center space-x-6">
                            <a href="{{ $socialFacebook }}" class="text-gray-400 hover:text-white transition-colors duration-200 transform hover:scale-110">
                                <span class="sr-only">{{ __('footer_facebook') }}</span>
                                <svg class="size-6" fill="currentColor" viewBox="0 0 24 24" aria-hidden="true">
                                    <path fill-rule="evenodd" d="M22 12c0-5.523-4.477-10-10-10S2 6.477 2 12c0 4.991 3.657 9.128 8.438 9.878v-6.987h-2.54V12h2.54V9.797c0-2.506 1.492-3.89 3.777-3.89 1.094 0 2.238.195 2.238.195v2.46h-1.26c-1.243 0-1.63.771-1.63 1.562V12h2.773l-.443 2.89h-2.33v6.988C18.343 21.128 22 16.991 22 12z" clip-rule="evenodd" />
                                </svg>
                            </a>
                            <a href="{{ $socialInstagram }}" class="text-gray-400 hover:text-white transition-colors duration-200 transform hover:scale-110">
        ...
        <div class="pb-16 lg:flex lg:items-center lg:justify-between lg:pb-24">
            <div class="space-y-8 lg:space-y-0 lg:flex lg:flex-1 lg:gap-12">
                <div>
                    <dl class="font-heading text-sm font-semibold uppercase tracking-wider text-gray-300 mb-6">
                        {{ __('footer_contact') }}
                    </dl>
                    <ul class="flex flex-col space-y-3 text-sm text-gray-400">
                        @if ($companyPhone)
                            <li class="flex items-center gap-3">
                                ...
                                <a href="tel:{{ $companyPhone }}" class="hover:text-white transition-colors duration-200">
                                    {{ $companyPhone }}
                                </a>
                            </li>
                        @endif
                        ...
                        @if ($companyEmail)
                            <li class="flex items-center gap-3">
                                ...
                                <a href="mailto:{{ $companyEmail }}" class="hover:text-white transition-colors duration-200">
                                    {{ $companyEmail }}
                                </a>
                            </li>
                        @endif
                ...
                <div>
                    ...
                    <dt class="text-sm leading-relaxed text-gray-400">
        ...
        <div class="flex flex-col items-center border-t border-gray-700/50 py-8 sm:flex-row sm:justify-between lg:py-12">
            <p class="text-sm text-gray-400">
                © {{ date('Y') }} {{ config('app.name') }}, Inc. {{ __('footer_all_rights_reserved') }}
                <a href="https://filamentphp.com" target="_blank" class="pl-1 underline hover:text-white transition-colors duration-200">
                    {{ __('footer_powered_by_filament') }}
                </a>
            </p>
            <div class="mt-8 flex items-center gap-6 divide-x divide-gray-700/50 sm:mt-0">
                <x-language-switcher />
                @if (auth()->check() && auth()->user()->can('view orders'))
                    <x-link href="{{ route('exports.index') }}"
                            class="inline-flex px-3 text-sm leading-5 text-gray-400 hover:text-white transition-colors duration-200">
                        {{ __('Exports') }}
                    </x-link>
                @endif
