...
<div class="wishlist-button" x-data='wishlistButton(@json([
    ...
    <button @click="toggleWishlist()"
            ...
            :title="isInWishlist ? '{{ __('frontend.wishlist.remove') }}' : '{{ __('frontend.wishlist.add') }}'"
            aria-label="isInWishlist ? '{{ __('frontend.wishlist.remove') }}' : '{{ __('frontend.wishlist.add') }}'">
        ...
        @if ($showText)
            <span class="ml-2 text-sm font-medium"
                  x-text="isInWishlist ? '{{ __('frontend.wishlist.saved') }}' : '{{ __('frontend.wishlist.save') }}'"></span>
        @endif
                ...
                try {
                    ...
                    if (response.ok) {
                        ...
                        this.message = data.message || (this.isInWishlist
                            ? '{{ __('frontend.wishlist.added') }}'
                            : '{{ __('frontend.wishlist.removed') }}');
                        ...
                        this.message = data.message || '{{ __('frontend.wishlist.errors.generic') }}';
                }
            } catch (error) {
                this.messageType = 'error';
                this.message = '{{ __('frontend.wishlist.errors.network') }}';
                } finally {
                    this.loading = false;
