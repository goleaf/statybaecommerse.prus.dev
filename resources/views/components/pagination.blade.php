...
@php
    ...
    // Preserve the current non-pagination query parameters so list navigation keeps filters active.
    $queryParameters = request()->query();
    unset($queryParameters['page']);

    // Helper closure that appends the serialized query string to any generated pagination URL.
    $appendQueryString = static function (?string $url) use ($queryParameters): ?string {
        if ($url === null) {
            return null; // Nothing to adjust when Filament renders a disabled pagination link.
        }

        if ($queryParameters === []) {
            return $url; // Skip work entirely if there are no filters to carry forward.
        }
...
@if ($lastPage > 1)
    <div class="bg-white border border-gray-200 rounded-xl p-6">
            ...
            <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-6">
                ...
                @if ($showPerPage)
                    <div class="flex items-center gap-2">
                        <label for="per-page" class="text-sm text-gray-700">{{ __('frontend.pagination.show') }}:</label>
                        <select id="per-page"
        ...
        <nav class="flex items-center justify-center" aria-label="{{ __('frontend.pagination.navigation') }}">
