...
<div @class([
    ...
    <div @class([
        ...
        @if ($product->hasImages())
            ...
            <img
                 ...
                 alt="{{ $imageAttrs['alt'] }}"
                 @class([
                     'h-64 w-full object-cover transition-transform duration-300 group-hover:scale-105' =>
                         $layout === 'grid',
        ...
        @else
            <div @class([
                'flex h-64 items-center justify-center bg-gray-200 dark:bg-gray-700' =>
                    $layout === 'grid',
                'flex h-full items-center justify-center bg-gray-200 dark:bg-gray-700' =>
                    $layout === 'list',
            ])>
        ...
        @if ($showWishlist || $showCompare)
            <div
                 class="absolute top-2 right-2 flex flex-col gap-2 opacity-0 group-hover:opacity-100 transition-opacity duration-300">
                @if ($showWishlist)
                    <button
    ...
    <div @class([
        ...
        @php
            $productSlug = $product->trans('slug');
            if (empty($productSlug) && !empty($product->slug)) {
                $productSlug = is_array($product->slug)
                    ? ($product->slug[app()->getLocale()] ?? reset($product->slug))
                    : $product->slug;
            }
        @endphp

        <h3 @class([
            'text-lg font-medium text-gray-900 dark:text-white line-clamp-2' =>
                $layout === 'grid',
            'text-xl font-medium text-gray-900 dark:text-white' => $layout === 'list',
        ])>
            <a href="{{ $productSlug ? route('product.show', $productSlug) : '#' }}"
               class="hover:text-blue-600 dark:hover:text-blue-400 transition-colors duration-200"
               @if (!$productSlug) aria-disabled="true" @endif>
                {{ $product->name }}
            </a>
        </h3>
        ...
        @if ($product->summary)
            <p @class([
                'mt-2 text-sm text-gray-600 dark:text-gray-300 line-clamp-2' =>
                    $layout === 'grid',
        ...
        @if ($showRating && $product->reviews_avg_rating)
            <div class="mt-2 flex items-center">
                <div class="flex items-center">
                    @for ($i = 1; $i <= 5; $i++)
                        <svg class="h-4 w-4 {{ $i <= $product->reviews_avg_rating ? 'text-yellow-400' : 'text-gray-300' }}"
