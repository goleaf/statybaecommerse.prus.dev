...
<div class="bg-white border border-gray-200 rounded-xl p-6" x-data="productFilters()">
    ...
    <form @submit.prevent="applyFilters" class="space-y-6">
        {{-- Price Range --}}
        <div>
            <h4 class="text-sm font-semibold text-gray-900 mb-3">{{ __('frontend.filters.price_range') }}</h4>
            <div class="space-y-3">
                ...
                @for ($i = 5; $i >= 1; $i--)
                    <label
                        ...
                        <div class="flex items-center gap-1">
                            ...
                            @endfor
                            <span class="text-sm text-gray-700 ml-2">{{ $i }}+ {{ trans_choice('common.stars', $i) }}</span>
                        </div>
                    </label>
                @endfor
            </div>
            ...
            <div class="space-y-2">
                ...
                <label
                    ...
                    <input type="checkbox"
                           ...
                           class="w-4 h-4 text-blue-600 border-gray-300 rounded focus:ring-blue-500">
                    <span class="text-sm text-gray-700">{{ __('frontend.filters.sort.on_sale') }}</span>
                </label>
                <label
                       class="flex items-center gap-3 cursor-pointer hover:bg-gray-50 p-2 rounded-lg transition-colors duration-200">
                    <input type="checkbox"
                           ...
                           class="w-4 h-4 text-blue-600 border-gray-300 rounded focus:ring-blue-500">
                    <span class="text-sm text-gray-700">{{ __('frontend.home.new_arrivals') }}</span>
                </label>
            </div>
        </div>
