...
<div class="order-tracking" x-data="orderTracking()">
    <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        @if ($order->shipping_address)
            <div class="bg-white border border-gray-200 rounded-2xl p-6 mb-8">
                <h2 class="text-xl font-semibold text-gray-900 mb-6">{{ __('ui.shipping_information') }}</h2>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    @if ($order->tracking_number)
                        <div>
                            <h3 class="font-semibold text-gray-900 mb-2">{{ __('ui.tracking_information') }}</h3>
                            <div class="space-y-2">
                                <div>
                                    <span class="text-sm text-gray-600">{{ __('ui.tracking_number') }}:</span>
                                ...
                                @if ($order->estimated_delivery)
                                    <div>
                                        <span class="text-sm text-gray-600">{{ __('ui.estimated_delivery') }}:</span>
                ...
                fetch('/orders/{{ $order->id ?? 0 }}/reorder', {
                    ...
                    .then(data => {
                        if (data.success) {
                            ...
                            alert('{{ __('ui.failed_to_reorder_items_please_try_again') }}');
                        }
                    })
                    .catch(error => {
                        alert('{{ __('ui.network_error_please_try_again') }}');
                    });
            }
        }
    }
