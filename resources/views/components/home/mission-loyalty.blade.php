			...
			<div class="grid grid-cols-1 gap-8">
				<div class="rounded-xl border border-white/15 bg-white/5 backdrop-blur-sm p-8 text-white">
                    ...
                    <div class="space-y-4 max-w-2xl">
						<span class="inline-flex items-center gap-2 rounded-full border border-white/20 px-4 py-1 text-xs font-semibold uppercase tracking-[0.3em] text-black">{{ __('home_mission_badge') }}</span>
						<h2 class="text-2xl sm:text-3xl font-bold">{{ __('home_mission_title') }}</h2>
						<p class="text-white/80 leading-relaxed">{{ __('home_mission_subtitle') }}</p>
                        <div class="flex flex-wrap items-center gap-3">
                            <a href="mailto:{{ config('app.email', 'support@statybae.com') }}" class="px-6 py-3 bg-sage font-semibold transition-all duration-300 transform hover:scale-105 flex items-center justify-center gap-2 relative overflow-hidden group rounded-md">
								...
								<span class="relative z-30 transition-colors duration-300 !text-black">{{ __('home_mission_consultation') }}</span>
							</a>
                            <a href="{{ Route::has('localized.collections.index') ? route('localized.collections.index', ['locale' => $locale]) : '#' }}" class="px-6 py-3 border-2 border-sage font-semibold transition-all duration-300 transform hover:scale-105 flex items-center justify-center gap-2 relative group rounded-md text-white hover:text-black">
								...
								<span class="relative z-10 transition-colors duration-300 text-white group-hover:text-black">{{ __('home_mission_view_solutions') }}</span>
							</a>
						</div>
					</div>
				</div>
				...
				<div class="rounded-xl border border-white/15 bg-white/5 backdrop-blur-sm p-8 text-white">
                    <div class="space-y-3">
						<span class="inline-flex items-center gap-2 rounded-full border border-white/20 px-4 py-1 text-xs font-semibold uppercase tracking-[0.3em] text-black">{{ __('home_loyalty_badge') }}</span>
						<h3 class="text-2xl font-bold">{{ __('home_loyalty_title') }}</h3>
						<p class="text-white/80">{{ __('home_loyalty_subtitle') }}</p>
					</div>
                    ...
                    <div class="mt-6 flex flex-wrap items-center gap-3">
                        <a href="{{ Route::has('register') ? route('register') : '#' }}" class="px-6 py-3 bg-sage font-semibold transition-all duration-300 transform hover:scale-105 flex items-center justify-center gap-2 relative overflow-hidden group rounded-md">
							...
							<span class="relative z-30 transition-colors duration-300 !text-black">{{ __('home_loyalty_join') }}</span>
						</a>
                        <a href="{{ Route::has('refer.friend') ? route('refer.friend') : (Route::has('register') ? route('register') : '#') }}" class="px-6 py-3 border-2 border-sage font-semibold transition-all duration-300 transform hover:scale-105 flex items-center justify-center gap-2 relative group rounded-md text-white hover:text-black">
							...
							<span class="relative z-10 transition-colors duration-300 text-white group-hover:text-black">{{ __('home_loyalty_invite_friend') }}</span>
						</a>
					</div>
					...
					<div class="mt-8 rounded-lg border-2 border-dashed border-white/25 p-6">
						<div class="flex items-start gap-3">
							...
							<div>
								<p class="text-white font-semibold">{{ __('home_loyalty_catalog_title') }}</p>
								<p class="text-white/75 text-sm">{{ __('home_loyalty_catalog_subtitle') }}</p>
							</div>
						</div>
					</div>
				</div>
