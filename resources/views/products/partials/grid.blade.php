            ...
            <a href="{{ route('frontend.products.show', $product) }}" class="flex flex-1 flex-col space-y-3">
                <div class="aspect-video w-full overflow-hidden rounded-lg bg-gray-100 dark:bg-slate-800">
                    @if ($product->thumbnail)
                        <img src="{{ $product->thumbnail }}" alt="{{ $product->trans('ui.name') ?? $product->name }}" class="h-full w-full object-cover">
                    @else
                ...
                <div class="space-y-1">
                    <p class="text-xs uppercase tracking-wide text-blue-600 dark:text-blue-300">
                        {{ optional($product->brand)?->trans('ui.name') ?? optional($product->brand)->name ?? __('ui.independent_brand') }}
                    </p>
                    ...
                    <p class="text-sm text-gray-600 dark:text-gray-300 line-clamp-2">
                        {{ strip_tags($product->trans('ui.short_description') ?? $product->short_description ?? $product->trans('ui.description') ?? $product->description) }}
                    </p>
                </div>
