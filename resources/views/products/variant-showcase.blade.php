            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                @foreach($products as $product)
                    <div class="border rounded-lg p-4 hover:shadow-md transition-shadow cursor-pointer"
                        ...
                        <h3 class="font-semibold text-gray-900 mb-2">{{ $product->trans('ui.name') ?? $product->name }}</h3>
                        <p class="text-sm text-gray-600 mb-2">{{ $product->trans('ui.short_description') ?? $product->short_description ?? $product->trans('ui.description') ?? $product->description }}</p>
        ...
        @if($selectedProduct)
            ...
            <div class="bg-white rounded-lg shadow-sm border p-6 mb-8">
                <div class="flex items-start justify-between mb-6">
                    <div class="flex-1">
                        <h2 class="text-2xl font-bold text-gray-900 mb-2">{{ $selectedProduct->trans('ui.name') ?? $selectedProduct->name }}</h2>
                        <p class="text-gray-600 mb-4">{{ $selectedProduct->trans('ui.description') ?? $selectedProduct->description }}</p>
                        ...
                        <div class="flex items-center gap-4">
                            ...
                            @if($selectedProduct->brand)
                                <span class="text-sm text-gray-500">
                                    {{ __('product.variants.showcase.brand') }}: {{ $selectedProduct->brand?->trans('ui.name') ?? $selectedProduct->brand->name }}
                                </span>
                            @endif
                        </div>
                    </div>
                    ...
                    @if($selectedProduct->main_image)
                        <img src="{{ $selectedProduct->main_image }}" 
                             alt="{{ $selectedProduct->trans('ui.name') ?? $selectedProduct->name }}"
