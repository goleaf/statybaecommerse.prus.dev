...
@section('title', $product->trans('ui.name') ?? $product->name)
        ...
        <nav class="mb-6 text-sm text-gray-500 dark:text-gray-400" aria-label="Breadcrumb">
            <ol class="flex flex-wrap items-center gap-1">
                ...
                @foreach ($product->categories as $category)
                    <li aria-hidden="true" class="px-1">/</li>
                <li><a href="{{ route('frontend.categories.show', $category) }}" class="hover:text-blue-600 dark:hover:text-blue-300">{{ $category->trans('ui.name') ?? $category->name }}</a></li>
                @endforeach
                <li aria-hidden="true" class="px-1">/</li>
                <li class="font-semibold text-gray-900 dark:text-white">{{ $product->trans('ui.name') ?? $product->name }}</li>
            </ol>
        </nav>
