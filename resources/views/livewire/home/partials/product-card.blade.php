@php
    ...
    $attributes = ($attributes ?? new ComponentAttributeBag())->merge([
        'class' => 'group relative flex h-full flex-col overflow-hidden rounded-3xl border border-gray-200 bg-white shadow-xl transition-all duration-300 hover:-translate-y-1 hover:shadow-2xl',
    ]);
@endphp
...
<div {{ $attributes }}>
    <div class="relative aspect-[4/3] overflow-hidden">
        ...
        @if ($product->brand)
            <span class="absolute top-4 right-4 rounded-full bg-white/90 px-3 py-1 text-xs font-medium text-gray-800 backdrop-blur">
                {{ $product->brand->name }}
            </span>
        @endif
    </div>
    ...
    <div class="flex flex-1 flex-col gap-4 px-6 py-6">
        <div class="space-y-2">
            <h3 class="text-lg font-semibold leading-tight text-gray-900 line-clamp-2">
                <a href="{{ route('product.show', $product->slug ?? $product) }}" class="transition hover:text-indigo-600">
                    {{ $product->name }}
                </a>
            </h3>
        ...
        <div class="mt-auto flex items-center justify-between gap-3">
            <a href="{{ route('product.show', $product->slug ?? $product) }}"
               class="inline-flex items-center gap-2 rounded-full border border-gray-300 bg-gray-50 px-4 py-2 text-xs font-semibold text-gray-700 transition hover:bg-gray-100">
