<section class="relative overflow-hidden bg-gray-50"
         ...
         aria-label="{{ __('home.hero_slider') }}">

    @if ($this->sliders->count() > 0)
        <!-- Slider Container -->
        <div class="relative h-96 md:h-[500px] lg:h-[600px]" aria-live="polite">
            @foreach ($this->sliders as $index => $slider)
                <div class="absolute inset-0 transition-opacity duration-1000 ease-in-out bg-var"
                     :class="{ 'opacity-100': currentSlide === {{ $index }}, 'opacity-0': currentSlide !==
                             {{ $index }} }"
                    ...
                    <div class="relative z-10 flex items-center justify-center h-full">
                        <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
                            <div class="space-y-6">
                                <!-- Title -->
                                <h1 class="text-3xl sm:text-4xl lg:text-6xl font-bold leading-tight text-var"
                                    data-text-color="{{ $slider->text_color }}">
                                    {{ $slider->getTranslatedTitle() }}
                                </h1>

                                <!-- Description -->
                                @if ($slider->getTranslatedDescription())
                                    <p class="text-lg sm:text-xl lg:text-2xl max-w-3xl mx-auto leading-relaxed text-var/90"
                                       data-text-color="{{ $slider->text_color }}">
                                ...
                                @if ($slider->getTranslatedButtonText() && $slider->button_url)
                                    <div class="pt-4">
                                        <a href="{{ $slider->button_url }}"
                                           ...
                                           aria-label="{{ $slider->getTranslatedButtonText() }}">
        ...
        @if ($this->sliders->count() > 1)
            ...
            <button @click="prevSlide()"
                    class="absolute left-4 top-1/2 -translate-y-1/2 z-20 bg-white/20 hover:bg-white/30 text-white p-3 rounded-full transition-all duration-300"
                    type="button"
            ...
            <button @click="nextSlide()"
                    class="absolute right-4 top-1/2 -translate-y-1/2 z-20 bg-white/20 hover:bg-white/30 text-white p-3 rounded-full transition-all duration-300"
                    type="button"
        ...
        @if ($this->sliders->count() > 1)
            <div class="absolute bottom-6 left-1/2 -translate-x-1/2 z-20 flex gap-2" role="tablist"
                 aria-label="{{ __('home.slider_pagination') }}">
                @foreach ($this->sliders as $index => $slider)
                    <button @click="goToSlide({{ $index }})"
                            ...
                            role="tab"
                            :aria-label="'{{ __('home.slide') }} ' + ({{ $index }} + 1)">
                    </button>
                @endforeach
            </div>
        @endif
        ...
        @if ($this->sliders->count() > 1)
            <div class="absolute top-4 right-4 z-20">
                <button @click="autoPlay = !autoPlay"
                        class="bg-white/20 hover:bg-white/30 text-white p-2 rounded-full transition-all duration-300"
                        type="button"
                        :aria-pressed="autoPlay"
                        aria-label="{{ __('home.slider_autoplay') }}">
                    ...
                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24" x-show="autoPlay">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                              d="M10 9v6l4-3-4-3zM21 12a9 9 0 11-18 0 9 9 0 0118 0z"></path>
                    </svg>
                </button>
            </div>
        @endif
    @else
        ...
        <div class="h-96 md:h-[500px] lg:h-[600px] flex items-center justify-center bg-gray-100">
            <div class="text-center">
                ...
                <p class="text-gray-500 text-lg">{{ __('home.no_slides') }}</p>
            </div>
        </div>
    @endif
</section>
