@section('meta')
    <x-meta
        :title="__('auth_ui.register.title') . ' - ' . config('app.name')"
        :description="__('auth_ui.register.subtitle')"
        ...
        <div class="flex h-full flex-col justify-between">
            ...
            <div class="mt-10 rounded-2xl border border-ash bg-white/70 p-6">
                <p class="text-sm text-slate-800">
                    {{ __('auth_ui.login.already_have_account') }}
    ...
    <div class="space-y-10">
        ...
        <form wire:submit="register" class="space-y-7">
            <div class="grid gap-5 sm:grid-cols-2">
                <div class="space-y-2">
                        <x-forms.label for="first_name" :value="__('auth_ui.register.first_name')" />
                    <x-forms.input
                        id="first_name"
                        type="text"
                        ...
                        autocomplete="given-name"
                            ...
                            placeholder="{{ __('auth_ui.register.first_name_placeholder') }}"
                    />
                    <x-forms.errors :messages="$errors->get('registrationForm.first_name')" class="mt-1" />
                </div>
                <div class="space-y-2">
                        <x-forms.label for="last_name" :value="__('auth_ui.register.last_name')" />
                    <x-forms.input
                        id="last_name"
                        type="text"
                        ...
                        autocomplete="family-name"
                            class="rounded-xl border border-ash bg-white px-4 py-3 text-base shadow-sm transition focus:border-dark focus:ring-2 focus:ring-dark/10"
                            placeholder="{{ __('auth_ui.register.last_name_placeholder') }}"
                    />
            ...
            <div class="grid gap-5 sm:grid-cols-2">
                <div class="space-y-2">
                    ...
                    <x-forms.errors :messages="$errors->get('registrationForm.password')" class="mt-1" />
                        <p class="text-xs text-slate-500">
                            {{ __('auth_ui.register.password_requirements') }}
                    </p>
                </div>

                <div class="space-y-2">
                        <x-forms.label for="password_confirmation" :value="__('auth_ui.register.password_confirm')" />
                    <x-forms.input
                ...
                <p>
                        {{ __('auth_ui.register.security_notice') }}
                </p>
            </div>
            ...
            >
                ...
                <span wire:loading class="inline-flex items-center gap-2">
                    <svg class="h-5 w-5 animate-spin" fill="none" viewBox="0 0 24 24">
                        ...
                        {{ __('auth_ui.register.creating_account') }}
                </span>
            </button>
        </form>
        </div>
            ...
            <div class="relative">
                ...
                <div class="relative flex justify-center">
                    <span class="bg-white px-4 text-xs font-semibold uppercase tracking-[0.3em] text-slate-500">
                        {{ __('auth_ui.register.or_login_with') }}
                    </span>
                </div>
            </div>
        ...
        <p class="text-center text-xs text-slate-500">
            {{ __('auth_ui.register.terms_agreement') }}
