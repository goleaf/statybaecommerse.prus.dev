...
### Bug Fixes
* Preloaded the Activity Log Filament table for Livewire rendering and aligned the accompanying feature test with an authenticated admin guard and ID-based assertions so listing checks succeed even alongside background activity records.
...
* Hardened catalogue search by querying the `product_categories` pivot, deriving product metrics without denormalised columns, soft-capping page sizes through validation, and tolerating missing Redis extensions so SQLite-backed API tests execute without fatal errors.
...
* Stabilized automated tests by switching Pest and PHPUnit to a shared on-disk SQLite database and resetting the refresh state before each run, eliminating missing-table failures for product factories.
...
### Bug Fixes
* Provisioned minimal Spatie permission and attribute value tables in the focused SQLite testing migrations so Filament feature suites no longer fail with missing-table errors during bootstrap.
* Ensured attribute validation rules persist plain text inputs in tests, avoiding double-encoded JSON strings that previously broke the feature assertions.
