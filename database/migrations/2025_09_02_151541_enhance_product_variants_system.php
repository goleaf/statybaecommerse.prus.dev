...
return new class extends Migration {
    ...
    {
        // Enhance product_variants table with size-dependent pricing (forward-only)
        if (Schema::hasTable('product_variants')) {
            Schema::table('product_variants', function (Blueprint $table): void {
                ...
                if (!Schema::hasColumn('product_variants', 'track_inventory')) {
                    // Use after('stock_quantity') if quantity column doesn't exist
        ...
        if (!Schema::hasTable('product_variant_attributes')) {
            Schema::create('product_variant_attributes', function (Blueprint $table): void {
                ...
                $table->foreign('attribute_value_id')->references('id')->on('attribute_values')->cascadeOnUpdate()->cascadeOnDelete();
                $table->unique(['variant_id', 'attribute_id'], 'variant_attribute_unique');
                $table->index(['variant_id', 'attribute_value_id']);
            });
        }

        // variant_pricing_rules
        if (!Schema::hasTable('variant_pricing_rules')) {
            Schema::create('variant_pricing_rules', function (Blueprint $table): void {
                $table->id();
                $table->unsignedBigInteger('product_id');
                $table->string('rule_name');
                $table->string('rule_type')->default('size_based');
                $table->json('conditions');
                $table->json('pricing_modifiers');
        ...
        if (!Schema::hasTable('variant_combinations')) {
            Schema::create('variant_combinations', function (Blueprint $table): void {
                $table->id();
                $table->unsignedBigInteger('product_id');
                $table->json('attribute_combinations');
