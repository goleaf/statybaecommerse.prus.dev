...
- Pest test bootstrap helpers now guard the `login()` helper with a function-existence check so repeated includes during `php artisan test` runs no longer trigger fatal redeclaration errors.
