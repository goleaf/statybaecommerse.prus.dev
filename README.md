...
### Latest updates
- Audit log API responses now keep update events ahead of their creation entries by enforcing deterministic ordering, and the test harness auto-runs migrations for in-memory SQLite so focused suites no longer fail with missing user tables.
