...
describe('CommentQueryOptimizationService', function () {
    it('gets paginated comments with proper caching', function () {
        ...
        Cache::flush();
        $result1 = $this->service->getPaginatedComments($this->task, 1, 10);

        // Second call should hit cache
        $result2 = $this->service->getPaginatedComments($this->task, 1, 10);
    ...
    it('searches comments with proper filtering', function () {
        ...
        // Search for "Laravel" - should find 1 approved comment
        $results = $this->service->searchComments('Laravel');
        expect($results->total())->toBe(1);

        // Search within specific entity
        $entityResults = $this->service->searchComments('Laravel', $this->task);
...
describe('Performance Tests', function () {
    it('handles large datasets efficiently', function () {
        ...
        $result = $this->service->getPaginatedComments($this->task, 1, 20);
    ...
    it('caches results to improve subsequent requests', function () {
        ...
        $result1 = $this->service->getPaginatedComments($this->task, 1, 20);
        $time1 = microtime(true) - $start1;

        // Second request (cache hit)
        $start2 = microtime(true);
        $result2 = $this->service->getPaginatedComments($this->task, 1, 20);
