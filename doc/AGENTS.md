...
- **Catalog & Merchandising**: core entities `app/Models/Product.php`, `app/Models/ProductVariant.php`, `app/Models/Brand.php`, `app/Models/Category.php`, supporting data transfer objects like `app/Data/ProductRequestData.php` and `app/Data/CatalogIntegrityReport.php`, importer/exporters under `app/Services/ImportExport/**`, pricing helpers in `app/Services/Pricing/**`, and admin resources like `app/Filament/Resources/Product*` and `app/Filament/Resources/Collection*`.
  ...
  - The `/api/categories/tree` endpoint reserves the literal `tree` slug via a regex constraint on the show route so cached tree responses win over implicit model binding; keep that guardrail intact when adjusting category routing.【F:routes/api.php†L68-L85】
  - Product image seeding now chunks through the full catalogue and stores a PNG fallback when GD is absent; preserve the chunking and fallback logic in `Database\Seeders\ProductImageSeeder` so seeded galleries stay in sync with the storefront during local testing.【F:database/seeders/ProductImageSeeder.php†L17-L136】
...
- **Automation & Analytics**: background orchestration in `app/Console/Commands/**`, event/listener wiring under `app/Listeners/**`, scheduled exports inside `app/Services/Export/**`, diagnostics in `app/Services/Debug/**`, analytics widgets/resources at `app/Filament/Resources/Analytics*`, and event capture in `app/Models/Analytics*.php`.
...
- **Placeholder test coverage**: The QA dashboards now reference the `DashboardFixtureTest` identifiers, with unit,
  ...
  entries even though the legacy class name was removed.
